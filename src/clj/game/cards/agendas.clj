...
(defn ice-boost-agenda [subtype]
  (letfn [(count-ice [corp]
    ...
    {:on-score {:msg (msg "gain " (count-ice corp) " [Credits]")
                ...
                :async true
                :effect (effect (gain-credits state side eid (count-ice corp)))}
...
(defn- agenda-counters
  ...
  ([qty ctype]
   {:effect (effect (add-counter state side eid card ctype qty nil))
    :async true
    :silent true}))
...
(defcard "15 Minutes"
  {:abilities [{:action true
                ...
                :effect (effect (move state :corp card :deck nil)
...
(defcard "Accelerated Beta Test"
  (letfn [(abt [choices]
            {:async true
             ...
             :effect (effect (wait-for (corp-install state side target nil
       ...
       {:async true
        ...
        :effect (effect (register-events
                       ...
                       [{:event :corp-shuffle-deck
                         :effect (effect (update! state side (assoc-in card [:special :shuffle-occurred] true)))}])
...
(defcard "Architect Deployment Test"
  {:on-score (combine-abilities
               ...
               {:prompt "Choose a card to install"
                :choices (effect (cancellable (filter corp-installable-type? (take 5 (:deck corp)))))
                :async true
                :change-in-game-state {:silent true
                                       :req (req (seq (:deck corp)))}
                :effect (effect (let [target-position (first (positions #{target} (take 5 (:deck corp))))]
   ...
   :abilities [{:cost [(->c :agenda 1)]
                ...
                :effect (effect (register-lingering-effect
                                  ...
                                  {:type :break-sub-additional-cost
                                   :duration :end-of-run
                                   :value (effect (repeat (count (:broken-subs (:ability context)))
                                                       (->c :trash-from-hand 1)))})
...
(defcard "Artificial Cryptocrash"
  {:on-score
   {:async true
    ...
    :effect (effect (lose-credits state :runner eid 7))}})
               ...
               :effect (effect (continue-ability
                                 state side (let [c (str->int target)]
                                   {:choices {:req (req (can-be-advanced? state target))}
                                    ...
                                    :async true
                                    :effect (effect (add-prop state :corp eid target :advance-counter c {:placed true}))})
                                 card nil))}})
          ...
          (interact [cards remaining to-trash to-add to-top stage]
            (cond
              ...
              {:prompt "Add a card to the top of R&D"
               :choices remaining
               :async true
               :effect (effect (continue-ability state side (interact cards (remove-card remaining target) to-trash to-add (conj to-top target) stage) card nil))}))]
    (let [arrange-rd
          {:interactive (effect true)
           ...
           :optional {:waiting-prompt true
                      ...
                      :yes-ability {:async true
                                    ...
                                    :choices ["OK"]
                                    :effect (effect (let [set-aside-cards (set-aside-for-me state side eid (take 7 (:deck corp)))]
                                                   (when (and
                                                           (:access @state)
                                                           (:run @state))
...
(defcard "Bellona"
  {:steal-cost-bonus (effect [(->c :credit 5)])
   ...
   {:optional
    {:waiting-prompt true
     ...
     {:prompt "How many credits do you want to pay?"
      :choices {:number (effect (min (:credit corp)
                                  (count (:hand runner))))}
      :async true
...
(defcard "Braintrust"
  ...
  {:static-abilities [{:type :rez-cost
                       :req (req (ice? target))
                       :value (effect (- (get-counters card :agenda)))}]})

(defcard "Breaking News"
  {:on-score (give-tags 2)
   :events (let [event {:unregister-once-resolved true
                        :req (req (first-event? state side :agenda-scored #(same-card? card (:card (first %)))))
                        :msg "make the Runner lose 2 tags"
                        :effect (effect (lose state :runner :tag 2))}]
     ...
     {:optional
      {:prompt "Take 1 bad publicity?"
       :yes-ability {:async true
                     ...
                     :effect (effect (wait-for (gain-bad-publicity state :corp 1)
                                            (agenda-counters state side card eid)))}
       :no-ability {:async true
                    :effect (effect (agenda-counters state side card eid))}}}
     :abilities [{:action true
  ...
  (letfn [(bucks [state]
    ...
    {:on-score
     {:async true
      ...
      :effect (effect (gain-credits state :corp eid (bucks state)))}}))
...
(defcard "Chronos Project"
  {:on-score
   {:req (req (not (zone-locked? state :runner :discard)))
    ...
    :interactive (effect true)
    :effect (effect (move-zone state :runner :discard :rfg))}})
  ...
  (letfn [(meat-damage [s c] (+ 2 (get-counters (get-card s c) :advancement)))]
    {:install-state :face-up
     :on-access {:req (req installed)
                 ...
                 :async true
                 :effect (effect (damage state side eid :meat (meat-damage state card) {:card card}))}}))
   ...
   {:interactive (effect true)
    ...
    {:prompt "Search R&D for a piece of ice to install protecting a remote server?"
     ...
     {:async true
      :effect (effect
                (continue-ability
                  state side (if (not-empty (filter ice? (:deck corp)))
                    {:async true
...
(defcard "Corporate War"
  {:on-score
   {:msg (msg (if (> (:credit corp) 6) "gain 7 [Credits]" "lose all credits"))
    :interactive (effect true)
    :async true
    :effect (effect (if (> (:credit corp) 6)
...
(defcard "Cyberdex Sandbox"
  {:on-score {:optional
   ...
   :events [{:event :purge
             ...
             :msg "gain 4 [Credits]"
             :async true
             :effect (effect (gain-credits state :corp eid 4))}]})
...
(defcard "Degree Mill"
  {:steal-cost-bonus (effect [(->c :shuffle-installed-to-stack 2)])})
            ...
            {:prompt "Choose a card to install"
             ...
             :choices {:card #(and (corp? %)
                                   ...
                                   (or (in-hand? %)
                                       (in-discard? %)))}
             :async true
             :effect (effect (wait-for (corp-install state side target server-name {:ignore-all-cost true
                                                                                 :msg-keys {:install-source card
                                                                                            :display-origin true}})
                      ...
                      (continue-ability
                        ...
                        {:optional
                         {:waiting-prompt true
                          ...
                          {:msg message
                           :async true
                           :effect (effect (wait-for (forfeit state card-side (make-eid state eid) card)
...
(defcard "Domestic Sleepers"
  {:agendapoints-corp (effect (if (pos? (get-counters card :agenda)) 1 0))
   :abilities [{:action true
                :cost [(->c :click 3)]
                :msg "place 1 agenda counter on itself"
                :async true
                :effect (effect (add-agenda-point-counters state side eid card 1))}]})
  ...
  {:events [{:event :corp-turn-ends
             ...
             :optional {:prompt "Search R&D for an Operation?"
                        ...
                        :req (req (pos? (get-counters card :agenda))
                                       (seq (:deck corp)))
                        :yes-ability {:choices (effect (cancellable (filter operation? (:deck corp)) :sorted))
...
(defcard "Eminent Domain"
  (let [expend-abi {:req (req (some corp-installable-type? (:hand corp)))
                    ...
                    :async true
                    :effect (effect (corp-install state side eid target nil
                                               {:install-state :rezzed
                                                :msg-keys {:install-source card
                                                           :display-origin true}
                                                :combined-credit-discount 5}))}
        score-abi {:interactive (effect true)
                   :optional {:prompt "Search R&D for 1 card to install and rez, ignoring all costs?"
                              :yes-ability {:async true
                                            :prompt "Choose a card to install"
...
(defcard "Escalate Vitriol"
  {:abilities [{:action true
                ...
                :async true
                :effect (effect (gain-credits state side eid (count-tags state)))}]})
...
(defcard "False Lead"
  (let [ab {:change-in-game-state {:req (req (<= 2 (:click runner)))}
            ...
            :cost [(->c :forfeit-self)]
            :effect (effect (lose-clicks state :runner 2))}]
    ...
    {:events [{:event :post-runner-turn-begins
     ...
     :abilities [ab
                 {:label "Ask when runner turn begins?"
                  ...
                  :effect (effect (update! state side (assoc-in card [:special :ask-when-runner-turn-starts] target))
                               (toast state :corp (str "False Lead prompt set to: " target) "warning"))}]}))

(defcard "Fetal AI"
  {:flags {:rd-reveal (effect true)}
   :on-access (assoc (do-net-damage 2) :req (effect (not (in-discard? card))))
   :steal-cost-bonus (effect [(->c :credit 2)])})

(defcard "Firmware Updates"
  {:on-score (agenda-counters 3)
   :abilities [{:cost [(->c :agenda 1)]
                :label "Place 1 advancement counter"
                :choices {:req (effect (and (ice? target)
...
(defcard "Freedom of Information"
  {:advancement-requirement (effect (- (count-tags state)))})
...
(defcard "Genetic Resequencing"
  {:on-score {:choices {:card in-scored?}
              ...
              :async true
              :effect (effect (wait-for (add-counter state side target :agenda 1 nil)
  ...
  (letfn [(graft [n] {:prompt "Choose a card to add to HQ"
    ...
    {:on-score
     {:async true
      ...
      :effect (effect (continue-ability state side (graft 1) card nil))}}))
...
(defcard "Hades Fragment"
  (let [abi {:prompt "Choose a card to add to the bottom of R&D"
    ...
    {:flags {:corp-phase-12 (effect (and (seq (:discard corp))
    ...
    :effect (effect (let [c (str->int target)]
                   (continue-ability
                     ...
                     {:choices {:card #(pos? (get-counters % :power))}
                      ...
                      :async true
                      :effect (effect (add-counter state side eid target :power c nil))}
                     card nil)))}})
...
(defcard "High-Risk Investment"
  {:on-score (agenda-counters 1)
   :abilities [{:action true
                ...
                :async true
                :keep-menu-open :while-agenda-tokens-left
                :effect (effect (gain-credits state side eid (:credit runner)))}]})
...
(defcard "Hollywood Renovation"
  {:install-state :face-up
   :events [{:event :advance
             ...
             :effect (effect (let [n (if (>= (get-counters (get-card state card) :advancement) 6) 2 1)]
                            (continue-ability
                              state side
                              {:choices {:not-self true
                               ...
                               :msg (msg "place " (quantify n "advancement counter")
                                         " on " (card-str state target))
                               :async true
                               :effect (effect (add-prop state :corp eid target :advance-counter n {:placed true}))}
                              card nil)))}]})
...
(defcard "Ikawah Project"
  {:steal-cost-bonus (effect [(->c :click 1) (->c :credit 2)])})
...
(defcard "Improved Protein Source"
  (let [ability {:async true
                 ...
                 :effect (effect (gain-credits state :runner eid 4))}]
    {:on-score ability
     :stolen ability}))

(defcard "Improved Tracers"
  {:move-zone (effect (if (and (in-scored? card)
...
(defcard "Jumon"
  {:events
   [{:event :corp-turn-ends
     ...
     :async true
     :effect (effect (add-prop state :corp eid target :advance-counter 2 {:placed true}))}]})
...
(defcard "Kimberlite Field"
  {:on-score
   {:interactive (effect true)
    ...
    :choices {:card #(rezzed? %)}
    :effect (effect (let [target-cost (:cost target)]
                   (wait-for (trash state side target {:cause-card card})
                             (continue-ability
                               ...
                               {:prompt (str "Choose a runner card that costs " target-cost " or less to trash")
                                ...
                                :msg (msg "trash " (:title target))
                                :async true
                                :effect (effect (trash state side eid target))}
                               card nil))))}})
  ...
  {:on-score (agenda-counters 2 :power)
   :prevention [{:prevents :jack-out
                 ...
                 :ability {:cost [(->c :power 1)]
                           ...
                           :effect (effect (wait-for (prevent-jack-out state side)
                                                  (register-lingering-effect
                                                    state side card
  ...
  (letfn [(move-to [c from]
            (choose-one-helper
              (let [and-then (fn [s] (str (if (= from :rd) ", shuffle R&D, and then " " and ") s))]
                ...
                [{:option "HQ"
                  :ability {:async true
                            :effect (effect (when (= from :rd) (shuffle! state side :deck))
                                         (move state side c :hand)
          ...
          (find-ab [zone]
            {:prompt "Choose an agenda"
             ...
             :choices (if (= zone :rd)
                        (effect (cancellable (filter agenda? (:deck corp)) :sorted))
                        {:card #(and (agenda? %) (if (= zone :hq) (in-hand? %) (in-discard? %)))})
             :effect (effect (continue-ability state side (move-to target zone) card nil))
             :async true
...
(defcard "License Acquisition"
  {:on-score {:interactive (effect true)
              ...
              :async true
              :effect (effect (corp-install state side eid target nil {:install-state :rezzed-no-cost
                                                            :msg-keys {:install-source card
                                                                       :display-origin true}}))}})
...
(defcard "Lightning Laboratory"
  (letfn [(ice-derez [zone]
            {:event :runner-turn-ends
             ...
             :async true
             :effect (effect (let [derez-count (min 2 (count (filter #(= (:zone %) [:servers zone :ices]) (all-active-installed state :corp))))]
                            (continue-ability
                              state side
    ...
    {:on-score (agenda-counters 1)
     :events [{:event :run
               ...
               {:prompt (msg "Remove 1 hosted agenda counter to rez up to 2 pieces of ice protecting " (zone->name (:server context)) ", ignoring all costs?")
                ...
                {:cost [(->c :agenda 1)]
                 :async true
                 :effect (effect (let [current-server (first (:server (:run @state)))]
                                (continue-ability
                                  state side
                                  {:prompt (msg "Choose up to 2 pieces of ice protecting " (zone->name current-server))
                                   ...
                                   :choices {:card #(and (ice? %)
                                             ...
                                             :max 2}
                                   :async true
                                   :cancel {:effect (effect (register-events state side card [(ice-derez current-server)]))}
                                   :effect (effect (register-events state side card [(ice-derez current-server)])
...
(defcard "Lotus Haze"
  {:on-score (agenda-counters 3)
   :abilities [{:cost [(->c :agenda 1)]
                ...
                ;;   --nbkelly, 2026.02
                :effect (effect (let [to-move target
                               ...
                               (continue-ability
                                 ...
                                 (if (seq legal-moves)
                                   {:prompt "Choose a server"
                                    :choices (effect legal-moves)
                                    :msg (msg "move " (:title to-move) " to " target)
                                    :effect (effect (let [c (move state side to-move
...
(defcard "Luminal Transubstantiation"
  {:on-score
   {:silent true
    :effect (effect (gain-clicks state :corp 3)
                 (register-turn-flag!
...
(defcard "Mandatory Seed Replacement"
  (letfn [(msr [] {:prompt "Choose two pieces of ice to swap positions"
    ...
    {:on-score {:async true
                ...
                :effect (effect (continue-ability state side (msr) card nil))}}))
...
(defcard "Mandatory Upgrades"
  {:move-zone (effect (if (and (in-scored? card)
                     ...
                     (do (system-msg state side (str "uses " (:title card) " to gain 1 addition [Click] per turn"))
                         ...
                         (effect-completed state side eid))
                     (effect-completed state side eid)))
   :leave-play (effect (lose state :corp
                          :click 1
                          :click-per-turn 1))})
...
(defcard "Market Research"
  {:on-score {:interactive (effect true)
              ...
              :async true
              :effect (effect (add-agenda-point-counters state side eid card 1))}
   :agendapoints-corp (effect (if (zero? (get-counters card :agenda)) 2 3))})
...
(defcard "Méliès City Luxury Line"
  {:steal-cost-bonus (effect [(->c :click 1)])
...
(defcard "Megaprix Qualifier"
  {:on-score {:silent true
              ...
              :async true
              :effect (effect (add-agenda-point-counters state side eid card 1))}
   :agendapoints-corp (effect (if (zero? (get-counters card :agenda)) 1 2))})

(defcard "Merger"
  {:agendapoints-runner (effect 3)})
...
(defcard "NAPD Contract"
  {:steal-cost-bonus (effect [(->c :credit 4)])
   :advancement-requirement (effect (count-bad-pub state))})

(defcard "Net Quarantine"
  (let [nq {:async true
            :effect (effect (let [extra (int (/ (:runner-spent target) 2))]
                           (if (pos? extra)
             ...
             {:req (req (same-card? card (:card context)))
              ...
              :yes-ability {:prompt "Choose a card to install"
                            ...
                            :effect (effect (corp-install
                                              state side eid target "New remote"
...
(defcard "NEXT Wave 2"
  {:on-score
   {:async true
    ...
    (effect
      (continue-ability
        state side (when (some #(and (rezzed? %)
                          (ice? %)
                          (has-subtype? % "NEXT"))
                    (all-installed state :corp))
          {:optional
...
(defcard "Oaktown Renovation"
  {:install-state :face-up
   :events [{:event :advance
             ...
             :async true
             :effect (effect (gain-credits state side eid (if (<= 5 (get-counters (get-card state card) :advancement)) 3 2)))}]})

(defcard "Obokata Protocol"
  {:steal-cost-bonus (effect [(->c :net 4)])})
...
(defcard "Off the Books"
  ...
  {:events [{:event :corp-turn-ends
             :interactive (effect true)
             :skippable true
             :optional {:req (req (pos? (get-counters card :agenda))
                                       (seq (:deck corp)))
                                                       ...
                                                       (continue-ability
                                                         ...
                                                         (choose-one-helper
                                                           [{:option (str "Install " (:title target-card))
                                                             :ability {:async true
                                                                       :effect (effect (corp-install state side eid target-card nil {:ignore-install-cost true :msg-args {:display-origin true :install-source card}}))}}
                                                            {:option (str "Add " (:title target-card) " to HQ")
                                                             :ability {:msg (msg "add " (:title target-card) " to HQ")
                                                                       :effect (effect (move state side target-card :hand))}}])
                                                         card nil))))}}}]})

(defcard "Ontological Dependence"
  {:advancement-requirement (effect (- (or (get-in @state [:runner :brain-damage]) 0)))})
   ...
   {:trace
    {:base 6
     ...
     {:msg "trash all connection and job resources"
      :async true
      :effect (effect (let [resources (filter #(or (has-subtype? % "Job")
...
(defcard "Personality Profiles"
  (let [pp {:req (req (pos? (count (:hand runner))))
    ...
    {:events [(assoc pp :event :searched-stack)
              (assoc pp
                     :event :runner-install
                     :req (effect (and (some #{:discard} (:previous-zone (:card context)))
...
(defcard "Philotic Entanglement"
  {:on-score {:interactive (effect true)
              ...
              :async true
              :effect (effect (damage state side eid :net (count (:scored runner)) {:card card}))}})
...
(defcard "Priority Requisition"
  {:on-score {:interactive (effect true)
              ...
              :async true
              :effect (effect (rez state side eid target {:ignore-cost :all-costs}))}})
...
(defcard "Profiteering"
  {:on-score {:interactive (effect true)
              ...
              :async true
              :effect (effect (let [bp (count-bad-pub state)]
...
(defcard "Project Beale"
  (project-agenda {:granularity 2})
  {:agendapoints-runner (effect 2)
   :agendapoints-corp (effect (+ 2 (get-counters card :agenda)))})
    ...
    {:events [{:event :corp-turn-ends
               ...
               :change-in-game-state {:silent true
                                      :req (req (some #(or (not (:seen %))
                                                           (not (operation? %)))
                                                      (:discard corp)))}
               :choices {:req (req (not (operation? target))
                                        (in-discard? target))}
               :async true
...
(defcard "Project Kusanagi"
  ...
  {:abilities [{:label "Give a piece of ice \"[Subroutine] Do 1 net damage\""
                ...
                :choices {:card #(and (ice? %)
                                      (rezzed? %))}
                :cost [(->c :agenda 1)]
                :change-in-game-state {:req (req run (some (every-pred ice? rezzed?)
                                                                (all-installed state :corp)))}
                :keep-menu-open :while-agenda-tokens-left
...
(defcard "Project Vacheron"
  {:flags {:has-events-when-stolen true}
   :agendapoints-runner (effect (if (or (= (first (:previous-zone card)) :discard)
   ...
   :events [{:event :runner-turn-begins
             ...
             :async true
             :effect (effect (if (pos? (get-counters card :agenda))
                            (wait-for
...
(defcard "Project Yagi-Uda"
  (letfn [(choose-swap [to-swap]
            {:async true
             ...
             :effect (effect (wait-for (swap-cards-async state side to-swap target)
          ...
          (choose-card [run-server]
            {:async true
             ...
             :effect (effect (continue-ability state side (choose-swap target) card nil))})]
    (project-agenda
      {:abilities [{:async true
                    :waiting-prompt true
                    :fake-cost [(->c :agenda 1)]
                    :keep-menu-open false ;; not using :while-agenda-tokens-left as the typical use case is only one token, even if there are multiple
                    :label "swap card in HQ with installed card"
                    :req (req run (can-pay? state side eid card nil [(->c :agenda 1)]))
                    :effect (effect (continue-ability state side (choose-card (:server run)) card nil))}]})))
...
(defcard "Proprionegation"
  {:on-score {:silent true
              :async true
              :effect (effect (add-counter state side eid card :agenda 1))}
   :abilities [{:req (req (:run @state))
                :cost [(->c :agenda 1)]
...
(defcard "Quantum Predictive Model"
  {:flags {:rd-reveal (effect true)}
   :on-access {:req (req tagged)
               ...
               :effect (effect (move state :corp card :scored {:force true})
                               (update-all-agenda-points state side)
                               (check-win-by-agenda state side))}})

(defcard "Rebranding Team"
  {:move-zone (effect (when (and (in-scored? card)
                              (= :corp (:scored-side card)))
...
(defcard "Reeducation"
  (letfn [(corp-final [chosen original]
            {:prompt (str "The bottom cards of R&D will be " (enumerate-cards chosen))
             ...
             :effect (effect (let [n (count chosen)]
                            (if (= target "Done")
    ...
    {:on-score {:async true
                :waiting-prompt true
                :effect (effect (let [from (get-in @state [:corp :hand])]
                               (if (pos? (count from))
...
(defcard "Regenesis"
  {:on-score {:req (req (some #(not (faceup? %)) (:discard corp))
...
(defcard "Regulatory Capture"
  {:advancement-requirement (effect (- (min 4 (count-bad-pub state))))})
...
(defcard "Research Grant"
  {:on-score {:interactive (effect true)
              :silent (effect (empty? (filter #(= (:title %) (:title card)) (all-installed state :corp))))
              :async true
              :effect (effect (continue-ability
                                state side {:prompt (str "Choose another installed copy of " (:title card) " to score")
                                 :choices {:card #(= (:title %) (:title card))}
                                 :interactive (effect true)
                                 :async true
                                 :req (req (seq (filter #(= (:title %) (:title card)) (all-installed state :corp))))
                                 :effect (effect (score state side eid (get-card state target) {:no-req true}))
...
(defcard "SDS Drone Deployment"
  {:steal-cost-bonus (effect [(->c :program 1)])
...
(defcard "Sericulture Expansion"
  ...
  {:events [(assoc (place-advancement-counter nil 2)
                   :event :corp-turn-ends
                   :req (effect (and (seq (all-installed state :corp))
...
(defcard "Sisyphus Protocol"
  (letfn [(rezzed-gate-or-sentry [context]
    ...
    {:events [{:event :pass-ice
               :req (req (rezzed-gate-or-sentry context)
                              (first-event? state side :pass-ice
                                            #(rezzed-gate-or-sentry (first %))))
               :async true
               :effect (effect (let [enc-ice (get-card state (:ice context))]
                              (continue-ability
                                state side
                                {:prompt (msg "Make the runner encounter " (:title (:ice context)) " again?")
                                 :choices (effect [(when (can-pay? state :corp eid card nil (->c :credit 1))
                                                  "Pay 1 [Credit]")
                                 ...
                                 :effect (effect (if (= target "Done")
                                                ...
                                                (continue-ability
                                                  ...
                                                  {:cost [(if (= target "Pay 1 [Credit]")
                                                   ...
                                                   :async true
                                                   :effect (effect (force-ice-encounter state side eid enc-ice))}
                                                  card nil)))}
                              card targets)))}]}))
   ...
   :events [{:event :corp-turn-begins
             ...
             {:req (req (pos? (get-counters card :credit)))
              ...
              {:async true
               ...
               :effect (effect (if (pos? (get-counters card :credit))
...
(defcard "Standoff"
  (letfn [(stand [side]
            {:async true
             ...
             :choices {:card #(and (installed? %)
                                   (same-side? side (:side %)))}
             :cancel {:display-side side
                      :msg "decline trashing any more cards"
                      :async true
                      :effect (effect (if (= side :runner)
             ...
             :effect (effect (wait-for (trash state side target
                                           (if (= side :corp)
    ...
    {:on-score
     {:interactive (effect true)
      :async true
      :effect (effect (show-wait-prompt state side (str (side-str (other-side side)) " to trash a card for Standoff"))
                      (continue-ability state :runner (stand :runner) card nil))}}))
...
(defcard "Sting!"
  (letfn [(count-opp-stings [state side]
    ...
    {:on-score {:msg (msg "deal " (inc (count-opp-stings state :corp)) " net damage")
                :async true
                :effect (effect (damage state side eid :net (inc (count-opp-stings state :corp)) {:card card}))}
     :stolen {:msg (msg "deal " (inc (count-opp-stings state :runner)) " net damage")
              :async true
              :effect (effect (damage state side eid :net (inc (count-opp-stings state :runner)) {:card card}))}}))
             ...
             :effect (effect (wait-for
                            ...
                            (continue-ability
                              ...
                              (assoc (place-advancement-counter nil 1)
                                     :req (effect (seq (all-installed-corp state))))
                              card nil)))})]
    {:on-score (score-abi 3)
     :derezzed-events [{:event :corp-install
                        ...
                        {:prompt "Reveal this agenda to gain 2 [Credits] and place 1 advancement counter on an installed card?"
                         :req (req (not= [:hand] (:previous-zone card))
...
(defcard "Successful Field Test"
  (letfn [(sft [n max-ops]
    ...
    {:on-score {:async true
                ...
                :effect (effect (let [max-ops (count (filter (complement operation?) (:hand corp)))]
   ...
   {:optional
    {:prompt "Draw 2 cards?"
     :yes-ability {:msg "draw 2 cards"
                   :async true
                   :effect (effect (draw state :corp eid 2))}}}})
...
(defcard "The Future is Now"
  {:on-score {:interactive (effect true)
              :prompt "Choose a card to add to HQ"
              :change-in-game-state {:req (effect (seq (:deck corp)))}
              :choices (effect (:deck corp))
   ...
   :abilities [{:cost [(->c :agenda 1)]
                ...
                :effect (effect
                          (continue-ability
                            state side (let [chosen-ice target]
                              {:prompt "Choose a server"
                               :choices (effect (installable-servers state chosen-ice))
                               :async true
                               :effect (effect
                                         (continue-ability
                                           state side (let [chosen-server target
                                             ...
                                             {:prompt "Which position to install in? (0 is innermost)"
                                              ...
                                              :async true
                                              :effect (effect (let [target (Integer/parseInt target)]
  ...
  (letfn [(adv4? [s c] (if (>= (get-counters (get-card s c) :advancement) 4) 2 1))]
    {:install-state :face-up
     :events [{:event :advance
               ...
               :effect (effect (mill state :corp eid :runner (adv4? state card)))}]}))
...
(defcard "Utopia Fragment"
  {:static-abilities
   [{:type :steal-additional-cost
     ...
     :value (effect (->c :credit (* 2 (get-counters target :advancement))))}]})
    ...
    (effect
      (register-events
        ...
        [{:event (if (= :corp (:active-player @state)) :corp-turn-ends :runner-turn-ends)
          ...
          :async true
          :effect (effect (damage state side eid :net (count (:hand runner)) {:card card}))}]))}})
...
(defcard "Vulnerability Audit"
  {:flags {:can-score (effect (let [result (not= :this-turn (installed? card))]
                             (when-not result
