  ...
  ([n counter-type]
   (let [num-counters (fn [card] (min n (get-counters card counter-type)))]
     {:msg (msg "gain " (num-counters card) " [Credits]")
      ...
      :async true
      :effect (effect (take-credits state side eid card counter-type n))})))
...
(defn creds-on-round-start
  ...
  (let [ability {:msg (str "gain " per-turn " [Credits]")
                 ...
                 :async true
                 :automatic :gain-credits
                 :effect (effect (gain-credits state side eid per-turn))}]
    {:derezzed-events [corp-rez-toast]
...
(defcard "Advanced Assembly Lines"
  {:on-rez {:async true
   ...
   :abilities [{:label "Install a non-agenda card from HQ"
                ...
                :change-in-game-state {:req (effect (seq (:hand corp)))}
                ...
                :choices {:card #(and (corp-installable-type? %)
                                      ...
                                      (corp? %))}
                :cost [(->c :trash-can)]
                :effect (effect (corp-install state side eid target nil {:msg-keys {:install-source card
                                                                         :display-origin true}}))}]})
...
(defcard "Aggressive Secretary"
  (advance-ambush 2 {:req (req (pos? (get-counters (get-card state card) :advancement)))
                     ...
                     :choices {:max (effect (get-counters (get-card state card) :advancement))
...
(defcard "Alix T4LB07"
  {:events [{:event :corp-install
   ...
   :abilities [{:action true
                ...
                :async true
                :effect (effect (gain-credits state side eid (* 2 (get-counters card :power))))}]})
  ...
  (letfn [(select-archives-cards [total]
            {:async true
             ...
             :choices {:card #(and (corp? %)
                       ...
                       :max total
                       :all true}
             :effect (effect (complete-with-result state side eid targets))})
    ...
    {:advanceable :always
     :abilities [{:label "Swap 1 card in HQ and Archives for each advancement counter"
                  ...
                  :async true
                  :waiting-prompt true
                  :effect (effect (let [total (min (count (:discard corp))
                                                (count (:hand corp))
                                                (get-counters card :advancement))]
...
(defcard "Amani Senai"
  (letfn [(senai-ability [agenda]
    ...
    {:events [{:event :agenda-scored
               :interactive (effect true)
               :async true
               :effect (effect (continue-ability state side (senai-ability (:card context)) card nil))}
              {:event :agenda-stolen
               :interactive (effect true)
               :async true
               :effect (effect (continue-ability state side (senai-ability (:card context)) card nil))}]
...
(defcard "Anson Rose"
  (let [ability {:label "Place 1 advancement counter (start of turn)"
    ...
    {:derezzed-events [corp-rez-toast]
     :flags {:corp-phase-12 (effect true)}
     :events [(assoc ability :event :corp-turn-begins)
              {:event :rez
               :req (req (ice? (:card context))
                              (pos? (get-counters card :advancement)))
               :async true
               :effect (effect (let [ice (get-card state (:card context))
                                  icename (:title ice)]
                              (continue-ability
                                state side
                                {:optional
                                 {:waiting-prompt true
                                  ...
                                  {:prompt "How many advancement counters do you want to move?"
                                   :choices {:number (effect (get-counters card :advancement))}
                                   :async true
...
(defcard "Anthill Excavation Contract"
  (let [ability {:once :per-turn
                 ...
                 :interactive (effect true)
                 :effect (effect (wait-for (draw state side 1 {:suppress-checkpoint true})
...
(defcard "API-S Keeper Isobel"
  (letfn [(counters-available? [state] (some #(pos? (get-counters % :advancement)) (all-installed state :corp)))]
    {:flags {:corp-phase-12 (effect (counters-available? state))}
     :abilities [{:req (req (:corp-phase-12 @state)
                  ...
                  :choices {:card #(and (pos? (get-counters % :advancement))
                                     (installed? %))}
                  :async true
                  :effect (effect (let [cnt (get-counters target :advancement)]
...
(defcard "B-1001" ;; note -> run restriction added by rules as errata
  {:abilities [{:req (req run (not this-server))
                :async true
                :cost [(->c :tag 1)]
                :msg "end the run"
...
(defcard "Balanced Coverage"
  (let [name-abi
        {:prompt "Choose a card type"
         ...
         :effect (effect (let [named-type target
                                      ...
                                      {:optional
                                       {:prompt "Reveal it to gain 2 [Credits]?"
                                        ...
                                        {:async true
                                         ...
                                         :effect (effect (wait-for (reveal state side (make-eid state eid) top-card)
                                                                (gain-credits state :corp eid 2)))}
                                        :no-ability {:effect (effect (system-msg state side (str "declines to use " (:title card) " to reveal the top card of R&D")))}}}
                                      card nil)
...
(defcard "Blacklist"
  {:on-rez {:effect (effect (lock-zone state side (:cid card) :runner :discard))}
   :leave-play (effect (release-zone state side (:cid card) :runner :discard))})
                 ...
                 :ability {:req (req (preventable? context))
                           :trace {:base 3
                                   :successful {:msg "prevent all bad publicity"
                                                :async true
                                                :effect (effect (prevent-bad-publicity state side eid :all))}}}}]})
   ...
   :on-access {:optional
               {:req (req (not (in-discard? card))
                ...
                :yes-ability {:async true
                              ...
                              :effect (effect (wait-for (gain-tags state :corp 1 {:suppress-checkpoint true})
                                                     (damage state side eid :net 3 {:card card})))}}}})

(defcard "C.I. Fund"
  {:derezzed-events [corp-rez-toast]
   :flags {:corp-phase-12 (effect (pos? (:credit corp)))}
   :abilities [{:label "Store up to 3 [Credit] (start of turn)"
                :prompt "How many credits do you want to store?"
                :once :per-turn
                :choices {:number (effect (min (:credit corp) 3))}
                :async true
                :effect (effect (wait-for (add-counter state side card :credit target nil)
   ...
   :events [{:event :corp-turn-begins
             ...
             :async true
             :effect (effect (add-counter state side eid card :credit 2 nil))}]})

(defcard "Calvin B4L3Y"
  {:abilities [(draw-abi 2 nil {:action true
                                :cost [(->c :click 1)]
                                :once :per-turn})]
   :on-trash {:interactive (effect true)
              :optional
              {:req (req (= :runner side))
               :waiting-prompt true
               :prompt "Draw 2 cards?"
...
(defcard "Cerebral Overwriter"
  (advance-ambush 3 {:async true
                     ...
                     :effect (effect (damage state side eid :brain (get-counters (get-card state card) :advancement) {:card card}))}))
...
(defcard "Charlotte Caçador"
  (let [choice-abi
        ...
        queue-ability {:interactive (effect true)
                       :skippable true
                       :event :corp-turn-begins
                       :req (req (not-used-once? state {:once :per-turn} card)
                                      (:corp-phase-12 @state))
                       :async true
                       :effect (effect (continue-ability state side choice-abi card nil))}
        trash-ab {:cost [(->c :advancement 1) (->c :trash-can)]
                  :label "Gain 3 [Credits]"
                  :msg (msg "gain 3 [Credits]")
                  :async true
                  :effect (effect (gain-credits state :corp eid 3))}]
    {:advanceable :always
     :flags {:corp-phase-12 (effect true)}
     :derezzed-events [corp-rez-toast]
...
(defcard "Chekist Scion"
  (advance-ambush 0 {:msg (msg "give the Runner " (quantify (inc (get-counters (get-card state card) :advancement)) "tag"))
                     :async true
                     :effect (effect (gain-tags state :corp eid (inc (get-counters (get-card state card) :advancement))))}))

(defcard "Chief Slee"
  {:events [{:event :end-of-encounter
             :req (req (pos? (count (remove :broken (:subroutines (:ice context))))))
             :msg (effect (let [unbroken-count (count (remove :broken (:subroutines (:ice context))))]
                         (str "place " (quantify unbroken-count "power counter") " on itself")))
             :async true
             :effect (effect (add-counter state :corp eid card :power (count (remove :broken (:subroutines (:ice context)))) nil))}]
   :abilities [{:action true
                :cost [(->c :click 1) (->c :power 5)]
                :keep-menu-open :while-5-power-tokens-left
                :async true
                :msg "do 5 meat damage"
                :effect (effect (damage state side eid :meat 5 {:card card}))}]})
...
(defcard "Clone Suffrage Movement"
  {:derezzed-events [corp-rez-toast]
   :flags {:corp-phase-12 (effect (and (some operation? (:discard corp))
                                 unprotected))}
   :abilities [(into
...
(defcard "Cohort Guidance Program"
  {:flags {:corp-phase-12 (effect true)}
   ...
   :events [{:event :corp-turn-begins
             ...
             :prompt "Choose one"
             :interactive (effect true)
             :choices (effect [(when (seq (:hand corp)) "Trash 1 card from HQ to gain 2 [Credits] and draw 1 card")
                                ...
                                {:prompt "Choose a card to trash"
                                 ...
                                 :choices {:max 1
                                           ...
                                           :card #(and (corp? %)
                                                       (in-hand? %))}
                                 :async true
                                 :effect (effect (wait-for (trash-cards state side targets {:cause-card card})
                                 ...
                                 :effect (effect (update! state side (assoc target :seen true))
                                              (continue-ability
                                                ...
                                                {:prompt "Choose an installed card"
                                                 ...
                                                 :async true
                                                 :effect (effect
                                                           (add-prop state side eid target
                                                                     :advance-counter 1
                                                                     {:placed true}))}
                                                card nil))})
                              card nil)))}]})
...
(defcard "Constellation Protocol"
  {:derezzed-events [corp-rez-toast]
   :flags {:corp-phase-12
           (effect (let [a-token (->> (all-installed state :corp)
                                   (filter ice?)
   ...
   :abilities [{:label "Move an advancement counter between 2 pieces of ice"
                ...
                :effect (effect
                          (continue-ability
                            state side (let [from-ice target]
                              {:prompt "Choose a piece of ice that can be advanced"
                               ...
                               :msg (msg "move an advancement counter from "
                                         ...
                                         (card-str state target))
                               :async true
                               :effect (effect (wait-for (add-prop state :corp target :advance-counter 1 {:placed true})
  ...
  {:advanceable :always
   :abilities [{:action true
               ...
               {:action true
                ...
                :effect (effect (damage state side eid :meat 2 {:card card}))}]})
...
(defcard "Corporate Town"
  (let [ability {:label "Trash a resource"
                 ...
                 :effect (effect (trash state side eid target {:unpreventable true :cause-card card}))}]
    {:derezzed-events [corp-rez-toast]
     :additional-cost [(->c :forfeit)]
     :flags {:corp-phase-12 (effect (and (rezzed? card)
...
(defcard "Cybersand Harvester"
  {:events [{:event :rez
             ...
             :msg "place 2 [Credits] on itself"
             :async true
             :effect (effect (add-counter state :corp eid card :credit 2 nil))}]
  ...
  {:derezzed-events [corp-rez-toast]
   :events [(first-time-draw-bonus :corp 1)
            {:event :corp-draw
             ...
             :interactive (effect true)
             :silent (effect (let [dbs (filter #(and (= "Daily Business Show" (:title %))
                                                  (rezzed? %))
                                            (all-installed state :corp))]
                            (not= card (first dbs))))
             :async true
             :effect (effect (let [dbs (count (filter #(and (= "Daily Business Show" (:title %))
                                                         (rezzed? %))
                                                   (all-installed state :corp)))
                              ...
                              (when (seq drawn)
                                {:waiting-prompt true
                                 ...
                                 :choices {:max (min dbs (count drawn))
                                           ...
                                           :all true}
                                 :effect (effect (doseq [c (reverse targets)]
  ...
  (let [ability {:req (req (not (some (into #{}
    ...
    {:rez-req (effect (= (:active-player @state) :corp))
     :events [{:event :successful-run
               :req (req this-server)
               :async true
               :effect (effect (system-msg state :runner (str "gains 2 [Credits] for a successful run "
...
(defcard "Dedicated Response Team"
  {:events [(assoc (do-meat-damage 2)
                   :event :run-ends
                   :req (effect (and tagged (:successful target))))]})
    ...
    {:abilities [{:action true
     ...
     :static-abilities [{:type :install-cost
                         :req (req (runner? target)
                                        (not-triggered? state))
                         :value (effect (get-counters card :power))}]
...
(defcard "Early Premiere"
  {:derezzed-events [corp-rez-toast]
   :flags {:corp-phase-12 (effect (some #(and (can-be-advanced? state %)
                                           (in-server? %))
                                     (all-installed state :corp)))}
...
(defcard "Echo Chamber"
  {:abilities [{:action true
                ...
                :effect (effect (as-agenda state :corp card 1))}]})
...
(defcard "Edge of World"
  (letfn [(ice-count [state]
    ...
    (installed-access-trigger 3 {:msg (msg "do " (ice-count state) " core damage")
                                 :async true
                                 :effect (effect (damage state side eid :brain (ice-count state)
                                                         {:card card}))})))
...
(defcard "Eliza's Toybox"
  {:abilities [{:action true
                ...
                :async true
                :effect (effect (rez state side eid target {:ignore-cost :all-costs :msg-keys {:include-cost-from-eid eid}}))}]})

(defcard "Elizabeth Mills"
  {:on-rez {:msg "remove 1 bad publicity"
            :effect (effect (lose-bad-publicity state side 1))}
   :abilities [{:action true
                :cost [(->c :click 1) (->c :trash-can)]
                :label "Trash a location and take 1 bad publicity"
                :async true
                :effect (effect (if (some #(has-subtype? % "Location") (all-installed state :runner))
                               (continue-ability
                                 state side
                                 {:prompt "Trash a location and take 1 bad publicity"
                                  ...
                                  :async true
                                  :effect (effect (wait-for (trash state side target {:cause-card card})
...
(defcard "Esca"
  {:flags {:rd-reveal (effect true)}
   ...
   :on-access {:msg "force the Runner to lose 1 [Credits]"
               :async true
               :effect (effect (wait-for (lose-credits state :runner 1)
                                      (continue-ability
                                        state side
                                        {:req (req tagged)
                                         :msg "do 1 net damage"
                                         :async true
                                         :effect (effect (damage state side eid :net 1))}
                                        card nil)))}})
...
(defcard "Estelle Moon"
  {:events [{:event :corp-install
   ...
   :abilities [{:label "Draw 1 card and gain 2 [Credits] for each hosted power counter"
                ...
                :async true
                :effect (effect (let [counters (get-counters card :power)
                                   credits (* 2 counters)]
...
(defcard "Executive Search Firm"
  {:abilities [{:action true
                ...
                :msg (msg "reveal " (:title target) ", add it to HQ, and shuffle R&D")
                :choices (effect (cancellable (filter #(has-any-subtype? % ["Executive" "Sysop" "Character"])
                                                   (:deck corp))
                                           :sorted))
                :cost [(->c :click 1)]
...
(defcard "Exposé"
  {:advanceable :always
   :abilities [{:label "Remove 1 bad publicity for each advancement counter on Exposé"
                ...
                :cost [(->c :trash-can)]
                :effect (effect (lose-bad-publicity state side (get-counters card :advancement)))}]})
  ...
  (letfn [(tag-count [false-flag]
    ...
    {:advanceable :always
     :on-access {:req (req (pos? (get-counters (get-card state card) :advancement)))
                 ...
                 :async true
                 :effect (effect (gain-tags state :corp eid (tag-count (get-card state card))))}
     :abilities [{:action true
                  ...
                  :effect (effect (as-agenda state :corp card 3))}]}))
...
(defcard "Federal Fundraising"
  (let [draw-ab {:optional {:req (req unprotected)
                            ...
                            :no-ability {:effect (effect (system-msg state side (str "declines to use " (:title card) " to draw 1 card")))}}}
        ability
        {:once :per-turn
         :req (req (:corp-phase-12 @state)
                        (not-empty (:deck corp)))
           ...
           state side {:optional
            {:prompt "Look at the top 3 cards of R&D?"
             ...
             {:msg "rearrange the top 3 cards of R&D"
              :async true
              :waiting-prompt true
              :effect (effect (let [from (take 3 (:deck corp))]
...
(defcard "Front Company"
  {:static-abilities [{:type :cannot-run-on-server
   ...
   :events [{:event :run
             ...
             :msg "do 2 net damage"
             :async true
             :effect (effect (damage state side eid :net 2))}]})

(defcard "Full Immersion RecStudio"
  {:can-host (effect (and (or (asset? target) (agenda? target))
                       (> 2 (count (:hosted card)))))
   :trash-cost-bonus (effect (* 3 (count (:hosted card))))
   :abilities [{:action true
                ...
                :async true
                :effect (effect (corp-install state side eid target card nil))}
...
(defcard "Fumiko Yamamori"
  {:events [{:event :reveal-spent-credits
             ...
             :effect (effect (damage state side eid :meat 1 {:card card}))}]})
...
(defcard "Gaslight"
  (let [search-for-operation {:prompt "Choose an operation to add to HQ"
                              ...
                              :choices (effect (conj (vec (sort-by :title (filter operation? (:deck corp)))) "Done"))
                              :async true
                              :effect (effect (if (= target "Done")
                   ...
                   (continue-ability
                     state side {:optional
                      {:prompt "Trash this asset to search R&D for an operation?"
                       :yes-ability
                       {:async true
                        :effect (effect (wait-for (trash state side card {:cause-card card})
...
(defcard "Gene Splicer"
  {:advanceable :always
   :on-access {:req (req (pos? (get-counters (get-card state card) :advancement)))
               ...
               :async true
               :effect (effect (damage state side eid :net (get-counters (get-card state card) :advancement)
                                       {:card card}))}
   :abilities [{:action true
...
(defcard "Genetics Pavilion"
  {:on-rez {:msg (msg "prevent the Runner from drawing more than 2 cards during [runner-pronoun] turn")
            :effect (effect (max-draw state :runner 2)
                         (when (zero? (remaining-draws state :runner))
                           (prevent-draw state :runner)))}
   :events [{:event :runner-turn-begins
             :silent true
             :effect (effect (max-draw state :runner 2))}]
   :leave-play (effect (swap! state update-in [:runner :register] dissoc :max-draw :cannot-draw))})
...
(defcard "Ghost Branch"
  (advance-ambush 0 {:async true
                     ...
                     :effect (effect (gain-tags state :corp eid (get-counters (get-card state card) :advancement)))}))
...
(defcard "GRNDL Refinery"
  {:advanceable :always
   :abilities [{:action true
                ...
                :async true
                :effect (effect (gain-credits state side eid (* 4 (get-counters card :advancement))))}]})
...
(defcard "Honeyfarm"
  {:flags {:rd-reveal (effect true)}
   ...
   :on-access {:msg "force the Runner to lose 1 [Credits]"
               :async true
               :effect (effect (lose-credits state :runner eid 1))}})
...
(defcard "Hostile Architecture"
  (letfn [(valid-ctx?
    ...
    {:events [{:event :runner-trash
               ...
               :msg "do 2 meat damage"
               :effect (effect (damage state :corp eid :meat 2 {:card card}))}]}))
...
(defcard "Hostile Infrastructure"
  {:events [{:event :runner-trash
             ...
             :msg "do 1 net damage"
             :effect (effect (damage state :corp eid :net 1 {:card card}))}]})

(defcard "Humanoid Resources"
  (let [play-an-instant
        {:prompt "Choose an operation"
         :choices (effect (conj (filter #(and (operation? %)
...
(defcard "Hyoubu Research Facility"
  {:events [{:event :reveal-spent-credits
             :req (req (some? (:corp-credits context))
                            (first-event? state side :reveal-spent-credits))
             :msg (msg "gain " (:corp-credits context) " [Credits]")
             :async true
             :effect (effect (gain-credits state :corp eid (:corp-credits context)))}]})
...
(defcard "Ibrahim Salem"
  (let [trash-ability (fn [card-type]
        ...
        choose-ability {:label "Trash 1 card in the grip of a named type"
                        :change-in-game-state {:req (effect (seq (:hand runner))) :silent true}
                        :once :per-turn
                        ...
                        :msg (msg "choose " target)
                        :async true
                        :effect (effect (continue-ability state side (trash-ability target) card nil))}]
    {:additional-cost [(->c :forfeit)]
     :flags {:corp-phase-12 (effect (not (is-disabled-reg? state card)))}
     :derezzed-events [corp-rez-toast]
    ...
    (let [abi {:event :corp-turn-begins
               ...
               :optional {:prompt "Trash Idiosyncresis?"
                          ...
                          :yes-ability {:async true
                                        ...
                                        :cost [(->c :trash-can)]
                                        :effect (effect (wait-for (lose-credits state :runner (lose card runner))
                           ...
                           {:optional
                            {:prompt (msg "Swap " (:title scored) " for an agenda in the Runner's score area?")
                             ...
                             {:prompt (str "Choose a scored Runner agenda to swap with " (:title scored))
                              ...
                              :async true
                              :effect (effect (let [new-scored (second (swap-agendas state side scored target))]
                                             (continue-ability
                                               state side
...
(defcard "Investigator Inez Delgado A 2"
  (letfn [(swap-abi [stolen]
    ...
    {:events [{:event :agenda-stolen
               ...
               :skippable true
               :async true
               :effect (effect (let [stolen (:card context)]
                ...
                :effect (effect (register-lingering-effect
                                  ...
                                  (let [it-target target]
                                    {:type :ice-strength
                                     ...
                                     :value (effect (inc (get-counters card :power)))}))
...
(defcard "Jackson Howard"
  {:abilities [(draw-abi 2 nil {:action true
               ...
               {:label "Shuffle up to 3 cards from Archives into R&D"
                ...
                :async true
                :effect (effect (shuffle-into-rd-effect state side eid card 3))}]})
...
(defcard "Janaína \"JK\" Dumont Kindelán"
  (let [ability {:label "Place 3 [Credits] on this asset (start of turn)"
                 ...
                 :async true
                 :effect (effect (add-counter state side eid card :credit 3 {:placed true}))}]
    {:derezzed-events [corp-rez-toast]
     :flags {:corp-phase-12 (effect true)}
     :events [(assoc ability :event :corp-turn-begins)]
     :abilities [ability
                 {:action true
                  ...
                  :effect (effect (when (pos? (get-counters (get-card state card) :credit))
...
(defcard "Jeeves Model Bioroids"
  (let [ability {:label "Gain [Click]"
        ...
        cleanup (effect (update! state side (dissoc card :seen-this-turn)))]
    {:abilities [ability]
     :leave-play cleanup
     :events [{:event :corp-spent-click
               :async true
               :effect (effect (let [{:keys [action value ability-idx]} context
...
(defcard "Kala Ghoda Real TV"
  {:derezzed-events [corp-rez-toast]
   ...
   :abilities [{:msg "look at the top card of the stack"
                ...
                :async true
                :effect (effect (continue-ability
                                  state side {:prompt (effect (->> runner :deck first :title (str "The top card of the stack is ")))
                                   :waiting-prompt true
                                   :choices ["OK"]}
                                  card nil))}
               {:async true
                ...
                :cost [(->c :trash-can)]
                :effect (effect (mill state :corp eid :runner 1))}]})

(defcard "Kuwinda K4H1U3"
  {:x-fn (effect (get-counters card :power))
   :derezzed-events [corp-rez-toast]
   :flags {:corp-phase-12 (effect true)}
...
(defcard "Lady Liberty"
  {:abilities [{:action true
                ...
                :effect (effect (let [c (move state :corp target :scored)]
...
(defcard "Lily Lockwell"
  {:on-rez (draw-abi 3)
   :abilities [{:action true
                ...
                :choices (effect (conj (vec (sort-by :title (filter operation? (:deck corp)))) "No action"))
                :async true
                :effect (effect (if (= target "No action")
                               (do (shuffle! state :corp :deck)
                                   (effect-completed state side eid))
                               (wait-for
                                 (reveal state side target)
            ...
            {:event :breach-server
             ...
             :effect (effect (continue-ability
                            ...
                            {:req (req
                             ...
                             :effect (effect (access-bonus state :runner (:server context) -1))}
                            card targets))}]})

(defcard "Luana Campos"
  {:uninstall (effect (continue-ability
                     state side
                     {:req (req (rezzed? (:old-card context))
...
(defcard "Malia Z0L0K4"
  (let [unmark
        (effect (when-let [malia-target (get-in card [:special :malia-target])]
    ...
    {:on-rez {:msg (msg "blank the text box of " (card-str state target))
              ...
              :effect (effect (update! state side (assoc-in (get-card state card) [:special :malia-target] target))
                           (update-disabled-cards state))}
     :leave-play unmark
     :move-zone unmark
     :static-abilities [{:type :icon
                        ...
                        :value (effect (make-icon "MZ" card))}
                        {:type :disable-card
                         :req (req
                                (let [malia-target (get-in (get-card state card) [:special :malia-target])]
                                  (or (same-card? target malia-target)
...
(defcard "Marilyn Campaign"
  (let [ability {:once :per-turn
                 :interactive (effect (>= 2 (get-counters card :credit)))
                 :req (req (:corp-phase-12 @state))
    ...
    {:derezzed-events [corp-rez-toast]
     ...
     :prevention [{:prevents :trash
                   ...
                   :ability {:msg "shuffle itself into R&D instead of moving it to Archives"
                             ...
                             :effect (effect (swap! state update-in [:prevent :trash :remaining] (fn [ctx] (mapv #(if (same-card? card (:card %)) (assoc % :destination :deck :shuffle-rd true) %) ctx))))}}]}))
  ...
  (let [ability (take-n-credits-start-of-turn 1)]
    {:abilities [ability
                 {:action true
                  ...
                  :msg "store 3 [Credits]"
                  :async true
                  :effect (effect (add-counter state side eid card :credit 3 nil))}]
     :events [(assoc ability :event :corp-turn-begins)]}))
...
(defcard "MCA Austerity Policy"
  {:abilities [{:action true
                ...
                :async true
                :effect (effect (register-events state side card
               ...
               {:action true
                ...
                :msg "gain 4 [Click]"
                :effect (effect (gain-clicks state side 4))}]})
    ...
    (let [moon-pool-reveal-ability
          {:prompt "Choose up to 2 facedown cards from Archives to shuffle into R&D"
           ...
           :msg (msg "reveal " (enumerate-cards targets :sorted) " from Archives and shuffle "
                     ...
                     " into R&D")
           :effect (effect (wait-for (reveal state side targets)
                                  (doseq [c targets]
                                    (move state side c :deck))
                                  (shuffle! state side :deck)
          ...
          {:prompt "Choose up to 2 cards from HQ to trash"
           ...
           :effect (effect (wait-for (trash-cards state :corp targets {:cause-card card})
                                  (continue-ability
                                    state side
           ...
           :cancel {:msg "decline to trash any cards from HQ"
                    :async true
                    :effect (effect (continue-ability state side moon-pool-reveal-ability card nil))}}]
      {:abilities [{:label "Trash up to 2 cards from HQ. Shuffle up to 2 cards from Archives into R&D"
                    :cost [(->c :remove-from-game)]
                    :async true
                    :effect (effect (continue-ability
                                      state side moon-pool-discard-ability
                                      card nil))}]})))
...
(defcard "Mr. Stone"
  {:events [{:event :runner-gain-tag
             :async true
             :msg "do 1 meat damage"
             :effect (effect (damage state :corp eid :meat 1 {:card card}))}]})
...
(defcard "Mumbad City Hall"
  {:abilities [{:action true
                ...
                :choices (effect (cancellable (filter #(and (has-subtype? % "Alliance")
                                                         (if (operation? %)
                                                           (<= (:cost %) (:credit corp))
                                                           true))
                                                   (:deck corp))
...
(defcard "Mumbad Construction Co."
  {:derezzed-events [corp-rez-toast]
   ...
   :abilities [{:cost [(->c :credit 2)]
                ...
                :async true
                :effect (effect (wait-for (add-prop state side card :advance-counter -1 {:placed true})
                                       (add-prop state side eid target :advance-counter 1 {:placed true})))}]})

(defcard "Museum of History"
  {:derezzed-events [corp-rez-toast]
   :flags {:corp-phase-12 (effect (pos? (count (get-in @state [:corp :discard]))))}
   :abilities [{:label "Shuffle cards in Archives into R&D"
                ...
                :choices {:card #(and (corp? %)
                                      (in-discard? %))
                          :max (effect (count (filter #(and (= (:title card) (:title %))
                                                         (rezzed? %))
                                                   (all-installed state :corp))))}
                :show-discard true
                :once :per-turn
  ...
  {:abilities [{:action true
   ...
   :on-trash {:optional
              {:req (req (= :runner side))
               ...
               {:msg "gain 2 [Credits]"
                :async true
                :effect (effect (gain-credits state :corp eid 2))}}}})
    ...
    {:implementation "Manual - click NASX to place power counters on itself"
     ...
     :abilities [ability
                 ...
                 {:label "Place 2 power counters"
                  ...
                  :async true
                  :effect (effect (add-counter state side eid card :power 2 nil))}
                 {:action true
...
(defcard "Net Analytics"
  (let [ability {:optional
    ...
    {:events [(-> ability
                  ...
                  (assoc-in [:optional :req] (effect (= (:side context) :runner))))
              (-> ability
                  (assoc :event :runner-prevent)
                  (assoc-in [:optional :req] (effect (= :tag (:type context)))))]
     :abilities [(set-autoresolve :auto-fire "Net Analytics")]}))

(defcard "Net Police"
  {:x-fn (effect (get-link state))
   :recurring (get-x-fn)
...
(defcard "Neurostasis"
  (advance-ambush
    ...
    {:req (req (pos? (get-counters (get-card state card) :advancement)))
     ...
     :choices {:card #(and (installed? %)
                           (runner? %))
               :max (effect (get-counters (get-card state card) :advancement))}
...
(defcard "News Team"
  {:flags {:rd-reveal (effect true)}
   ...
   :on-access {:async true
               ...
               :effect (effect (if (= target "Take 2 tags")
...
(defcard "NGO Front"
  (letfn [(builder [cost cred]
            {:cost [(->c :advancement cost) (->c :trash-can)]
             :async true
             :effect (effect (gain-credits state side eid cred))
...
(defcard "Nightmare Archive"
  {:flags {:rd-reveal (effect true)}
   ...
   :on-access {:async true
               ...
               :effect (effect (if (= target "Suffer 1 core damage")
  ...
  {:data {:counter {:power 3}}
   :events [(trash-on-empty :power)
            ...
            {:event :corp-turn-begins
             ...
             :async true
             :effect (effect (wait-for (lose-bad-publicity state :corp 1 {:suppress-checkpoint true})
...
(defcard "Pālanā Agroplex"
  (let [ability {:msg "make each player draw 1 card"
                 ...
                 :automatic :draw-cards
                 :async true
                 :effect (effect (wait-for (draw state :corp 1)
                              ...
                              (continue-ability
                                ...
                                {:optional {:prompt (str "Gain " creds-to-gain " [Credits]?")
                                            ...
                                            :yes-ability {:msg (str "gain " creds-to-gain " [Credits]")
                                                          :async true
                                                          :effect (effect (gain-credits state side eid creds-to-gain))}}}
                                card nil))))}]})
              ...
              (merge
                {:option (str "Do " x " net damage")
                 :ability {:msg (str "do " x " net damage")
                           :async true
                           :effect (effect (damage state :corp eid :net x))}}
                (if (= x 1)
                  {}
                  {:cost [(->c :power (dec x))]})))
              ...
              {:optional
               {:prompt (msg "Reveal and install " (:title agenda) "?")
                ...
                :no-ability {:async true
                             :effect (effect (continue-ability state side (pdhelper (next agendas)) card nil))}}}))]
  ...
  {:prevention [{:prevents :damage
                 ...
                 :ability {:async true
                           ...
                           :req (req (= :net (:type context))
                                          ...
                                          (preventable? context))
                           :effect (effect (wait-for (prevent-damage state side 1)
   ...
   :abilities [{:action true
                ...
                :async true
                :effect (effect (damage state side eid :net (get-counters card :power) {:card card}))}]})
...
(defcard "Project Junebug"
  (advance-ambush 1 {:req (req (pos? (get-counters (get-card state card) :advancement)))
                     ...
                     :async true
                     :effect (effect (damage state side eid :net (* 2 (get-counters (get-card state card) :advancement))
                                             {:card card}))}))

(defcard "Psychic Field"
  (let [ab {:async true
            :req (req installed)
            :effect (effect (let [hand (count (:hand runner))
                           ...
                           (continue-ability
                             ...
                             {:psi {:not-equal
                                    {:msg message
                                     :async true
                                     :effect (effect (damage state side eid :net hand {:card card}))}}}
                             card nil)))}]
    {:on-expose ab
     :on-access ab}))
...
(defcard "Public Access Plaza"
  (assoc (creds-on-round-start 1)
         :on-trash (assoc (give-tags 1) :req (effect (and (= :runner side) (threat-level 2 state))))))
  ...
  {:data {:counter {:power 3}}
   ...
   :events [{:event :corp-turn-begins
            ...
            {:event :counter-added
             ...
             :msg "add itself to [their] score area as an agenda worth 1 agenda point"
             :effect (effect (as-agenda state side card 1))}]})
  ...
  (letfn [(rez-ice [cnt discount]
            {:prompt (str "Choose a piece of ice to rez, paying " discount " [Credits] less")
             ...
             :choices {:req (req (ice? target)
                                      ...
                                      (not (rezzed? target)))}
             :waiting-prompt true
             :effect (effect (wait-for (rez state side target {:no-warning true :cost-bonus (- discount)})
                                    (if (< cnt 3)
                                      (continue-ability
                                        state side
  ...
  {:events [{:event :corp-draw
             ...
             {:prompt "Swap two cards?"
              ...
              {:once :per-turn
               ...
               :effect
               (effect
                 (lose-clicks state :corp 1)
                 (continue-ability
                   state side {:prompt "Choose a card in HQ that you just drew to swap for a card of the same type in Archives"
                    :choices {:card #(some (fn [c] (same-card? c %)) corp-currently-drawing)}
                    :async true
                    :effect
                    (effect
                      (continue-ability
                        state side (let [set-aside-card target
...
(defcard "Reality Threedee"
  (let [ability {:effect (effect (gain-credits state side eid (if tagged 2 1)))
                 :async true
...
(defcard "Reconstruction Contract"
  {:events [{:event :damage
             ...
             :msg "place 1 advancement counter on itself"
             :async true
             :effect (effect (add-counter state side eid card :advancement 1 {:placed true}))}]
   :abilities [{:label "Move hosted advancement counters to another card"
                ...
                :choices {:number (effect (get-counters card :advancement))
                          :default (effect (get-counters card :advancement))}
                :effect (effect (let [num-counters target]
                               (continue-ability
                                 state side
                                 {:async true
                                  :prompt "Choose a card that can be advanced"
                                  :choices {:req (req (can-be-advanced? state target))}
                                  :effect (effect (system-msg state side (str "uses " (:title card) " to move " (quantify num-counters "hosted advancement counter") " to " (card-str state target)))
                                                  (add-counter state side eid target :advancement num-counters {:placed true}))}
                                 card nil)))}]})
...
(defcard "Reversed Accounts"
  {:advanceable :always
   :abilities [{:action true
                ...
                :async true
                :effect (effect (lose-credits state :runner eid (* 4 (get-counters card :advancement))))}]})
...
(defcard "Rex Campaign"
  (let [payout-ab {:prompt "Choose one"
                   ...
                   :async true
                   :effect (effect (if (= target "Remove 1 bad publicity")
        ...
        ability {:once :per-turn
                 ...
                 :async true
                 :effect (effect (wait-for (add-counter state side card :power -1 nil)
...
(defcard "Ronin"
  {:advanceable :always
   :abilities [(assoc (do-net-damage 3)
                      :action true
                      :cost [(->c :click 1) (->c :trash-can)]
                      :req (effect (>= (get-counters card :advancement) 4)))]})
...
(defcard "Sandburg"
  {:on-rez {:effect (effect (update-all-ice state side))}
   :static-abilities [{:type :ice-strength
                       ...
                       :value (effect (quot (:credit corp) 5))}]
   :events [{:event :corp-gain
             :req (req (= :credit (:type context)))
             :effect (effect (update-all-ice state side))}
            {:event :corp-lose
             :req (req (= :credit (:type context)))
             :effect (effect (update-all-ice state side))}]
   :leave-play (effect (update-all-ice state side))})

(defcard "Sealed Vault"
  {:abilities [{:label "Store any number of credits"
                :cost [(->c :credit 1)]
                :prompt "How many credits do you want to move?"
                :choices {:number (effect (- (:credit corp) 1))}
                :msg (msg "store " target " [Credits]")
                :async true
                :effect (effect (wait-for (add-counter state side card :credit target)
                                       (lose-credits state side eid target)))}
               {:action true
                ...
                :msg (msg "gain " target " [Credits]")
                :async true
                :effect (effect (gain-credits state side eid target))}
               {:label "Move any number of credits to your credit pool"
                :prompt "How many credits do you want to move?"
                :choices {:counter :credit}
                :msg (msg "gain " target " [Credits]")
...
(defcard "Sensie Actors Union"
  {:derezzed-events [corp-rez-toast]
   :flags {:corp-phase-12 (effect unprotected)}
...
(defcard "Server Diagnostics"
  (let [ability {:effect (effect (gain-credits state side eid 2))
                 :async true
                 :once :per-turn
                 :automatic :gain-credits
...
(defcard "Shannon Claire"
  {:abilities [{:action true
                ...
                :effect (effect (play-sfx state side "click-card")
                                (move state side (last (:deck corp)) :hand))}
               {:label "Search R&D for an agenda"
                ...
                :choices (effect (cancellable (filter agenda? (:deck corp)) :sorted))
                :cost [(->c :trash-can)]
               ...
               {:label "Search Archives for an agenda"
                ...
                :choices (effect (cancellable (filter agenda? (:discard corp)) :sorted))
   ...
   {:optional
    {:req (req (not (in-deck? card)))
     ...
     {:prompt "How many credits do you want to pay?"
      ...
      :effect (effect
                (continue-ability
                  state side (let [dmg target]
                    {:player :runner
                     :prompt "Choose one"
                     :waiting-prompt true
                     :choices [(str "Take " dmg " net damage") "Add Shi.Kyū to score area"]
                     :async true
                     :effect (effect (if (str/starts-with? target "Add")
...
(defcard "Spin Doctor"
  {:on-rez {:async true
   ...
   :abilities [{:label "Shuffle up to 2 cards from Archives into R&D"
                :cost [(->c :remove-from-game)]
                :async true
                :effect (effect (shuffle-into-rd-effect state side eid card 2))}]})
...
(defcard "Storgotic Resonator"
  {:abilities [{:action true
   ...
   :events [{:event :corp-trash
             :once-per-instance true
             :req (req (some #(= (:faction (:identity runner)) (:faction (:card %))) targets)
                            (first-event?
...
(defcard "Superdeep Borehole"
  ...
  {:on-rez {:async true
            :effect (effect (update! state side (assoc-in (get-card state card) [:special :borehole-valid] true))
                         (add-counter state side eid card :bad-publicity 6 nil))}
   :events [{:event :corp-turn-begins
             :msg (msg "take 1 bad publicity from " (:title card))
             :async true
             :effect (effect (wait-for (add-counter state side card :bad-publicity -1 nil)
                                    (gain-bad-publicity state :corp eid 1)))}
            {:event :counter-added
             :req (req (same-card? card (:card context))
             ...
             :msg "win the game"
             :effect (effect (win state :corp (:title card)))}]})
...
(defcard "Sundew"
  {:events [{:event :runner-spent-click
             ...
             :msg "gain 2 [Credits]"
             :async true
             :effect (effect (update! state side (assoc-in card [:special :spent-click] true))
                          (gain-credits state :corp eid 2))}
            {:event :run
             :req (req (first-event? state side :runner-spent-click)
                            run
             ...
             :msg "lose 2 [Credits]"
             :async true
             :effect (effect (update! state side (dissoc-in card [:special :spent-click]))
...
(defcard "Svyatogor Excavator"
  (let [ability {:async true
                 ...
                 :effect (effect (wait-for (trash state side target {:unpreventable true
                                                                  :cause-card card})
                                        (gain-credits state side eid 3)))}]
    {:flags {:corp-phase-12 (effect (>= (count (all-installed state :corp)) 2))}
...
(defcard "Synth DNA Modification"
  {:events [{:event :subroutines-broken
             :req (req (has-subtype? (:ice context) "AP")
...
(defcard "Tech Startup"
  {:derezzed-events [corp-rez-toast]
   ...
   :abilities [{:label "Search R&D for an asset to install"
                ...
                :choices (effect (filter asset? (:deck corp)))
                :async true
                :effect (effect (wait-for
                               (trash state side card {:cause-card card})
                               (wait-for
...
(defcard "TechnoCo"
  (letfn [(is-techno-target [card]
    ...
    {:special {:auto-fire :always}
     ...
     :static-abilities [{:type :install-cost
                         :req (req (is-techno-target target)
                                        (not (:facedown (second targets))))
                         :value 1}]
     :events [{:event :runner-install
               :optional {:req (req (is-techno-target (:card context))
...
(defcard "Test Ground"
  {:advanceable :always
   :abilities [{:label "Derez 1 card for each advancement counter"
                ...
                :cost [(->c :trash-can)]
                :async true
                :effect (effect (let [cards-to-pick (min (count (filter #(and (rezzed? %) (not (agenda? %))) (all-installed state :corp)))
                                                      (get-counters card :advancement))
                                   payment-eid eid]
                               (continue-ability
                                 state side
                                 {:prompt (str "derez " cards-to-pick " cards")
                                  ...
                                  :choices {:card (every-pred installed? rezzed? (complement agenda?))
                                            ...
                                            :all true}
                                  :async true
                                  :effect (effect (derez state side eid targets {:msg-keys {:include-cost-from-eid payment-eid}}))}
                                 card nil)))}]})
...
(defcard "The News Now Hour"
  {:events [{:event :runner-turn-begins
             :silent true
             :effect (effect (prevent-current state side))}]
   :on-rez {:effect (effect (prevent-current state side))}
   :leave-play (effect (swap! state assoc-in [:runner :register :cannot-play-current] false))})
...
(defcard "Thomas Haas"
  {:advanceable :always
   :abilities [{:label "Gain credits"
                ...
                :cost [(->c :trash-can)]
                :async true
                :effect (effect (gain-credits state side eid (* 2 (get-counters card :advancement))))}]})
...
(defcard "Toshiyuki Sakai"
  (advance-ambush
    ...
    {:async true
     ...
     :effect (effect (let [counters (get-counters card :advancement)
                    ...
                    (continue-ability
                      ...
                      {:optional
                       {:prompt "Access the newly installed card?"
                        :yes-ability {:async true
                                      :effect (effect (access-card state side eid (get-card state moved-target)))}}}
                      moved-card nil)))}))
...
(defcard "Trieste Model Bioroids"
  {:on-rez {:msg (msg "prevent " (card-str state target)
            ...
            :choices {:card #(and (ice? %)
                                  (rezzed? %)
                                  (has-subtype? % "Bioroid"))}
            :effect (effect (update! state side (assoc-in (get-card state card) [:special :trieste-target] target)))}
   :static-abilities [{:type :icon
                       :req (req (same-card? target (get-in card [:special :trieste-target])))
                       :value (effect (make-icon "TMB" card))}
...
(defcard "Ubiquitous Vig"
  (let [ability {:msg (msg "gain " (get-counters card :advancement)  " [Credits]")
                 ...
                 :once :per-turn
                 :async true
                 :effect (effect (gain-credits state side eid (get-counters card :advancement)))}]
    {:derezzed-events [corp-rez-toast]
     :advanceable :always
     :events [(assoc ability :event :corp-turn-begins)]
     :abilities [ability]}))
...
(defcard "Urtica Cipher"
  (advance-ambush 0 {:msg (msg "do " (+ 2 (get-counters (get-card state card) :advancement)) " net damage")
                     :async true
                     :effect (effect (damage state side eid :net (+ 2 (get-counters (get-card state card) :advancement)) {:card card}))}))
...
(defcard "Vaporframe Fabricator"
  {:on-trash {:req (req (= :runner side))
              ...
              (effect
                (continue-ability
                  state side (let [card-to-install target]
                    {:async true
                     :prompt "Choose a server"
                     :choices (effect (remove (set (zone->name (get-zone card)))
                                           (installable-servers state card-to-install)))
                     :effect (effect (corp-install state side eid card-to-install target {:ignore-all-cost true
                                                                               :msg-keys {:install-source card
                                                                                          :display-origin true}}))})
                  card nil))}
   :abilities [{:action true
...
(defcard "Vera Ivanovna Shuyskaya"
  (let [ability {:interactive (effect true)
...
(defcard "Victoria Jenkins"
  {:on-rez {:effect (effect (lose state :runner :click-per-turn 1))}
   :leave-play (effect (gain state :runner :click-per-turn 1))
   :on-trash executive-trash-effect})
...
(defcard "Wage Workers"
  (let [payoff {:msg "gain [Click]"
    ...
    {:events [{:event :action-resolved
               :req (req (= :corp side))
               :async true
               :effect (effect (let [similar-actions
...
(defcard "Wall to Wall"
  (let [all [{:msg "gain 1 [Credits]"
        ...
        choice (fn choice [abis n]
                 (let [choices (concat (mapv make-label abis) ["Done"])]
                   {:prompt "Choose an ability to resolve"
                    :choices choices
                    :async true
                    :effect (effect (let [chosen (first (filter #(= % target) choices))
        ...
        ability {:async true
                 ...
                 :once :per-turn
                 :effect (effect (continue-ability state side (choice all (if (< 1 (count (filter asset? (all-active-installed state :corp))))
                                                                 1
                                                                 3)) card nil))}]
...
(defcard "Warden Fatuma"
  {:static-abilities [{:type :additional-subroutines
                       :req (req (ice? target) (rezzed? target) (has-subtype? target "Bioroid"))
...
(defcard "Warm Reception"
  (let [install {:prompt "Choose a card to install"
                 ...
                 :choices {:card #(and (corp-installable-type? %)
                                       (in-hand? %))}
                 :effect
                 (effect (wait-for (corp-install state side (make-eid state eid) target nil {:msg-keys {:install-source card
                                                                                                     :display-origin true}})
        ...
        derez-abi {:label "Derez another card (start of turn)"
                   ...
                   :choices {:not-self true
                             :card #(rezzed? %)}
                   :async true
                   :effect (effect (derez state side eid [card target]))}]
    {:derezzed-events [corp-rez-toast]
     :events [{:event :corp-turn-begins
               :interactive (effect true)
               :async true
               :effect (effect (wait-for (resolve-ability state side install card nil)
  ...
  {:events [{:event :rez
   ...
   :abilities [{:action true
               ...
               {:action true
                ...
                :async true
                :effect
                (effect (wait-for (gain-credits state side 6)
                               (continue-ability
                                 state side
...
(defcard "Worlds Plaza"
  {:abilities [{:action true
                ...
                :async true
                :effect (effect (wait-for (corp-install state side target card nil) ;; install target onto card
...
(defcard "Zaibatsu Loyalty"
  {:prevention [{:prevents :expose
                 ...
                 :ability {:cost [(->c :credit 1)]
                           ...
                           :async true
                           :effect (effect (prevent-expose state side eid card))}}
                {:prevents :expose
                 :type :ability
                                     ...
                                     (continue-ability
                                       ...
                                       {:optional
                                        {:req (req (not (rezzed? card)))
                                         ...
                                         :yes-ability {:async true
                                                       :effect (effect (rez state side eid card))}}}
                                       card nil)))}]})

(defcard "Zealous Judge"
  {:rez-req (effect tagged)
   :abilities [{:action true
                :async true
