  ...
  (auto-icebreaker
    {:abilities [(break-sub 1 1 "All" break-req)
                 ...
                 {:req (req (seq (filter #(has-subtype? % "Deva") (:hand runner))))
                  ...
                  :async true
                  :effect (effect (swap-cards-async state side eid card target))}]}))
...
(defn- install-from-heap
  ...
  {:abilities abilities
   ...
   :events [{:event :encounter-ice
             ...
             :effect (effect
                       (continue-ability
                         state side {:req (req (not-any? #(and (= title (:title %))
                                    ...
                                    (continue-ability
                                      ...
                                      (cond
                                        (= target "Yes") {:async true
                                                          :effect (effect (runner-install state :runner (assoc eid :source card :source-type :runner-install) card {:msg-keys {:install-source card :display-origin true}}))}
                                        ;; Add a register to note that the player was already asked about installing,
                                        ;; to prevent multiple copies from prompting multiple times.
                                        (= target "No") {:effect (effect (swap! state assoc-in [:run :register (keyword (str "conspiracy-" title)) (:cid current-ice)] true))}
...
(defn- pump-and-break
  ...
  (merge
    ...
    {:label (str "add " strength " strength and "
     ...
     :async true
     :effect (effect (pump state side card strength)
...
(defn- mu-based-strength
  ...
  [abilities]
  {:abilities abilities
   :static-abilities [(breaker-strength-bonus (effect (available-mu state)))]})
                   ...
                   state side {:optional
                    {:prompt (str "Pay " cost
                     ...
                     {:cost [(->c :credit cost)]
                      ...
                      :effect (effect (register-once state side {:once :per-turn} card)
...
(defn- virus-breaker
  ...
  (auto-icebreaker
    {:events [{:event :successful-run
               :silent true
               :async true
               :effect (effect (system-msg state side (str "places 1 virus counter on " (:title card)))
...
(defn- central-only
  ...
  (auto-icebreaker
    {:abilities [(break-sub (:break-cost break) (:break break) (:breaks break)
                            {:req (req (#{:hq :rd :archives} (target-server run))
    ...
    (auto-icebreaker
      {:abilities [break
                   ...
                   {:label (str "Derez " ice-type " being encountered")
                    ...
                    :async true
                    :effect (effect (derez state side eid current-ice {:msg-keys {:include-cost-from-eid eid}}))}]})))
  ...
  (auto-icebreaker
    (cloud-icebreaker
      {:abilities [(break-sub [(->c :trash-can)] 3 ice-type)]
       :static-abilities [(breaker-strength-bonus (effect (count (filter #(has-subtype? % "Icebreaker")
                                                                      (all-active-installed state :runner)))))]})))
  ...
  (auto-icebreaker {:abilities [(break-sub 1 1 "Code Gate")
                    ...
                    :events [{:event :subroutines-broken
                              ...
                              :effect (effect (continue-ability
                                                state side {:prompt "Choose 1 card in the grip to trash"
                                                 :waiting-prompt true
                                                 :async true
                                                 :choices {:card #(and (in-hand? %)
                                                                       (runner? %))}
...
(defcard "Aghora"
  (swap-with-in-hand "Aghora"
                     {:req (req (<= 5 (value (first (get-rez-cost state side current-ice nil))))
                                     (<= (get-strength current-ice) (get-strength card)))}))
  ...
  {:events
   [{:event :runner-turn-begins
    ...
    {:event :runner-turn-ends
     :async true
     :effect (effect (if (get-in card [:special :used-algernon])
                    (do
...
(defcard "Ankusa"
  (auto-icebreaker {:abilities [(break-sub 2 1 "Barrier")
                    ...
                    :events [{:event :subroutines-broken
                              ...
                              :effect (effect (move state :corp (:ice context) :hand nil)
                                           (continue state :runner nil))}]}))
  ...
  (auto-icebreaker
    {:abilities [(break-sub [(->c :virus 1)] 2)
     ...
     :events [{:event :runner-trash
               ...
               :async true
               :effect (effect (add-counter state :runner eid card :virus 1 nil))
  ...
  (auto-icebreaker {:implementation "[Erratum] Whenever you finish breaching a server, if you did not steal or trash any accessed cards, place 1 virus counter on this program."
                    :abilities [(break-sub 1 1)
                                {:label "Place 1 virus counter"
                                 ...
                                 :async true
                                 :effect (effect (add-counter state side eid card :virus 1 nil))}]
                    :static-abilities [(breaker-strength-bonus (effect (get-virus-counters state card)))]
                    :events [{:event :end-breach-server
                             ...
                             {:event :expose
                              :async true
                              :effect (effect (add-counter state side eid card :virus (count (:cards context)) nil))}]}))
...
(defcard "Baba Yaga"
  (let [gain-abis (effect (let [new-abis (mapcat (comp ability-init card-def) (:hosted card))]
  ...
  (letfn [(switch-server [key serv]
            {:option (str "Switch to " serv)
             ...
             :ability {:msg (str "change the attacked server to " serv)
                       :effect (effect (swap! state assoc-in [:run :server] [key]))}})]
...
(defcard "Begemot"
  (auto-icebreaker {:on-install {:async true
                    ...
                    :static-abilities [(breaker-strength-bonus (effect (:brain-damage runner)))]}))

(defcard "Berserker"
  (auto-icebreaker {:events [{:event :encounter-ice
                              :req (req (has-subtype? (:ice context) "Barrier"))
                              :msg (msg "gain " (count (:subroutines (:ice context))) " strength")
                              :effect (effect (pump state side card (count (:subroutines (:ice context)))))}]
...
(defcard "Bishop"
  {:implementation "[Erratum] Program: Caïssa - Trojan"
   :abilities [{:action true
                ...
                :async true
                :effect (effect (let [b (get-card state card)
                                   hosted? (ice? (:host b))
...
(defcard "Boi-tatá"
  (let [was-a-runner-card? (fn [target] (runner? (:card (first target))))
        discount-fn (effect (when (not (no-event? state side :runner-trash was-a-runner-card?))
                           [(->c :credit -1)]))]
    (auto-icebreaker
...
(defcard "Brahman"
  (auto-icebreaker {:abilities [(break-sub 1 2 "All")
                    ...
                    :events [{:event :end-of-encounter
                              ...
                              :effect (effect (move state side (get-card state target) :deck {:front true}))}]}))
   ...
   :events [{:event :corp-draw
             ...
             {:prompt "Pay credits to reveal drawn card?"
              ...
              {:prompt "How many cards do you want to reveal for 2 [Credits] each?"
               :waiting-prompt true
               :choices {:number (effect (min (:count context)
                                           (quot (total-available-credits state :runner eid card) 2)))}
               :async true
               :effect (effect (let [cards (->> corp-currently-drawing
                                             (shuffle)
...
(defcard "Carmen"
   (auto-icebreaker {:install-cost-bonus (effect (if (:successful-run runner-reg) -2 0))
...
(defcard "Chisel"
  ...
  {:implementation "[Erratum] Program: Virus - Trojan"
   :static-abilities [{:type :ice-strength
                       ...
                       :value (effect (- (get-virus-counters state card)))}]
   :events [{:event :encounter-ice
             :automatic :pre-bypass
             :req (req (same-card? (:ice context) (:host card)))
             :interactive (effect true)
             :async true
             :effect (effect (if (pos? (ice-strength state side (:ice context)))
...
(defcard "Cat's Cradle"
  (auto-icebreaker
    {:static-abilities [{:type :rez-cost
                         :req (req (ice? target) (has-subtype? target "Code Gate"))
                         :value 1}]
  ...
  {:static-abilities [{:type :cannot-score
   ...
   :events [trash-on-purge
            {:event :corp-install
             ...
             :effect (effect (swap! state update-in [:corp :register :cannot-score] #(cons (:card context) %)))}]})
  ...
  {:events [{:event :run-ends
             ...
             {:req (req (:successful context)
              ...
              :yes-ability {:msg "place 1 virus counter on itself"
                            :async true
                            :effect (effect (add-counter state side eid card :virus 1 nil))}
              :no-ability {:effect (effect (system-msg state side (str "declines to use " (:title card) " to place 1 virus counter on itself")))}}}
            {:event :successful-run
             :req (req (= :rd (target-server context))
                            this-card-run)
             :effect (effect (register-events
                              state side card [(breach-access-bonus :rd (max 0 (get-virus-counters state card)) {:duration :end-of-run})]))}]
...
(defcard "Consume"
  {:special {:auto-place-counter :always}
   :events [{:event :runner-trash
             ...
             :effect (effect (let [amt-trashed (count (filter #(corp? (:card %)) targets))
                                sing-ab {:optional {:prompt (msg "Place 1 virus counter on " (:title card) "?")
                                                    :autoresolve (get-autoresolve :auto-place-counter)
                                                    :yes-ability {:effect (effect (add-counter state :runner eid card :virus 1 nil))
                                                                  :async true
                                                                  :msg "place 1 virus counter on itself"}}}
                                mult-ab {:prompt (msg "Place virus counters on " (:title card) "?")
                                         ...
                                         :async true
                                         :effect (effect (add-counter state :runner eid card :virus target nil))}
   ...
   :abilities [{:action true
                ...
                :async true
                :effect (effect (play-tiered-sfx state side "click-credit" (* 2 (get-virus-counters state card)) 3)
...
(defcard "Copycat"
  {:abilities [{:async true
                ...
                :effect (effect (let [dest (second (get-zone target))
   ...
   :events [{:event :successful-run
             ...
             {:req (req (is-central? (target-server context))
              ...
              {:prompt "Choose a piece of ice protecting this server"
               ...
               :effect (effect
                         (continue-ability
                           state side (let [first-ice target]
                             {:prompt "Choose a piece of ice to swap with"
                              ...
                              :async true
                              :effect (effect (wait-for (pay state side (make-eid state eid) card [(->c :virus 1)])
...
(defcard "Corsair"
  (auto-icebreaker {:abilities [(break-sub 1 1 "Barrier")
                                {:label "Give -3 strength to encountered Barrier"
                                 ...
                                 :effect (effect (pump-ice state side current-ice -3))
                                 :cost [(->c :credit 1 {:stealth :all-stealth})]}]}))

(defcard "Cradle"
  (auto-icebreaker {:abilities [(break-sub 2 0 "Code Gate")]
                    :static-abilities [(breaker-strength-bonus (effect (- (count (:hand runner)))))]}))
  ...
  (auto-icebreaker {:abilities [(break-sub 1 1 "All")
                    ...
                    :events [{:event :end-of-encounter
                              ...
                              :msg (msg (if (can-pay? state side eid card nil [(->c :virus 1)])
                                          ...
                                          "trash itself"))
                              :async true
                              :effect (effect (wait-for (pay state :runner (make-eid state eid) card [(->c :virus 1)])
...
(defcard "Customized Secretary"
  (letfn [(custsec-host [cards]
    ...
    {:on-install {:async true
                  :interactive (effect (some #(card-flag? % :runner-install-draw true) (all-active state :runner)))
                  :msg (msg "reveal " (enumerate-cards (take 5 (:deck runner))) " from the top of the stack")
                  :waiting-prompt true
                  :effect (effect (let [from (take 5 (:deck runner))]
...
(defcard "Cyber-Cypher"
  (auto-icebreaker
    {:on-install {:prompt "Choose a server"
     ...
     :leave-play (effect (update! state side (dissoc card :card-target)))
...
(defcard "D4v1d"
  (auto-icebreaker
    (let [david-req (effect (<= 5 (get-strength current-ice)))]
...
(defcard "Darwin"
  (auto-icebreaker {:flags {:runner-phase-12 (effect true)}
                    :x-fn (effect (get-virus-counters state card))
...
(defcard "Datasucker"
  {:events [{:event :successful-run
   ...
   :abilities [{:cost [(->c :virus 1)]
                ...
                :effect (effect (pump-ice state side current-ice -1))}]})
  ...
  {:events
   [{:event :initialize-trace
     ...
     {:waiting-prompt true
      ...
      :yes-ability
      {:cost [(->c :trash-can)]
       :effect (effect (force-base state 0))}}}]})
...
(defcard "Djinn"
  {:static-abilities [{:type :can-host
                       :req (req (<= (expected-mu state target) 3)
...
(defcard "Eater"
  (auto-icebreaker {:abilities [(break-sub 1 1 "All" {:additional-ability {:msg "access not more than 0 cards for the remainder of this run"
                                                                           :effect (effect (max-access state 0))}
                                                      :label "break 1 subroutine and access 0 cards"})
                                (strength-pump 1 1)]}))

(defcard "Echelon"
  (auto-icebreaker {:static-abilities [(breaker-strength-bonus (effect (count (filter #(and (program? %)
  ...
  (let [force-draw (fn [c]
                     {:optional
                      {:prompt (str "Force the Corp to draw " (:title c) "?")
                       :yes-ability
                       {:async true
                        :effect (effect (system-msg state :corp (str "is forced to draw " (:title c)))
                                     (draw state :corp eid 1))}}})
        rvl {:optional
             {:prompt "Reveal the top card of R&D?"
              :yes-ability
              {:async true
               :effect (effect (let [topcard (get-in @state [:corp :deck 0])]
                              (wait-for
    ...
    {:events [{:event :successful-run
               ...
               :interactive (effect true)
               :waiting-prompt true
               :effect (effect (continue-ability state side rvl card nil))}]}))
...
(defcard "Faerie"
  (auto-icebreaker {:abilities [(break-sub 0 1 "Sentry")
                    ...
                    :events [{:event :end-of-encounter
                              ...
                              :effect (effect (trash state side eid card {:cause :runner-ability
                                                               :cause-card card}))}]}))
                      ...
                      (continue-ability
                        ...
                        (let [ice (:ice context)]
                          {:async true
                           ...
                           :waiting-prompt true
                           :player :corp
                           :choices (effect [(when (can-pay? state :runner eid card nil [(->c :credit (rez-cost state side ice))])
                                            (str "Rez " (get-title ice)))
                                          (str "Add " (get-title ice) " to HQ")])
                           :effect (effect (if (= target (str "Rez " (get-title ice)))
...
(defcard "Femme Fatale"
  (auto-icebreaker
    {:on-install {:prompt "Choose a piece of ice to target for bypassing"
                  ...
                  :effect (effect (update! state side (assoc-in (get-card state card) [:special :femme] target)))}
     :static-abilities [{:type :icon
                         :req (req (same-card? target (get-in card [:special :femme])))
                         :value (effect (make-icon "FF" card))}]
     :events [{:event :encounter-ice
               :skippable true
               :interactive (effect true)
               :change-in-game-state {:silent true
                                      :req (effect (let [ice (get-card state (:ice context))]
...
(defcard "Fermenter"
  {:data {:counter {:virus 1}}
   ...
   :abilities [{:action true
                ...
                :async true
                :effect (effect (play-tiered-sfx state side "click-credit" (* 2 (get-virus-counters state card)) 3)
                                (gain-credits state side eid (* 2 (get-virus-counters state card))))}]})
...
(defcard "Flux Capacitor"
  ...
  {:events [{:event :subroutines-broken
             :once :per-encounter
             :async true
             :req (req this-server (same-card? current-ice (:host card)))
...
(defcard "Gauss"
  (auto-icebreaker {:static-abilities [(breaker-strength-bonus (effect (= :this-turn (installed? card))) 3)]
                    :abilities [(break-sub 1 1 "Barrier")
                                (strength-pump 2 2)]}))
...
(defcard "God of War"
  (auto-icebreaker
    (let [abi {:label "Take 1 tag to place 2 virus counters (start of turn)"
               ...
               :effect (effect (add-counter state side eid card :virus 2 nil))}]
...
(defcard "Gorman Drip v1"
  {:events [{:event :corp-credit-gain
   ...
   :abilities [{:action true
                ...
                :label "Gain credits"
                :async true
                :effect (effect (gain-credits state side eid (get-virus-counters state card)))
...
(defcard "Grappling Hook"
  (let [break-subs (fn [state ice subroutines]
    ...
    {:abilities [{:label "break all but 1 subroutine"
                  ...
                  :choices (effect (unbroken-subroutines-choice current-ice))
                  ...
                  :async true
                  :effect (effect (let [selected (:idx (first targets))
...
(defcard "Gravedigger"
  (let [e {:req (req (installed? (:card target))
                          (corp? (:card target)))
...
(defcard "Harbinger"
  {:on-trash
   {:req (req (not-any? #{:facedown :hand} (get-zone card)))
    :effect (effect (if (zone-locked? state :runner :discard)
                              ...
                              {:optional
                               {:prompt "Host a card on this program instead of accessing it?"
                                :yes-ability {:prompt "Choose a card in Archives"
                                              :choices (effect (cancellable (:discard corp) :sorted))
                                              :msg (msg "host " (:title target) " on itself instead of accessing it")
                                              :effect (effect
                                                        (update! state side (assoc-in card [:special :host-available] false))
                                ...
                                {:optional
                                 {:prompt "Host face-down card on this program instead of accessing it?"
                                  :yes-ability {:msg "host a facedown card on itself instead of accessing it"
                                                :effect (effect (update! state side (assoc-in card [:special :host-available] false))
                                                                (host state side card target-card))}}}
   ...
   :abilities [{:action true
                ...
                :effect (effect (continue-ability
                               ...
                               {:waiting-prompt true
                                ...
                                :choices (effect (filter corp? (:hand corp)))
                                :async true
...
(defcard "Hivemind"
  {:data {:counter {:virus 1}}
   :abilities [{:req (req (pos? (get-counters card :virus)))
                ...
                :async true
                :effect (effect (wait-for (add-counter state :runner target :virus 1 {:suppress-checkpoint true})
...
(defcard "Imp"
  {:data {:counter {:virus 2}}
   :interactions {:access-ability {:label "Trash card"
                                   ...
                                   :once :per-turn
                                   :async true
                                   :effect (effect (trash state side eid (assoc target :seen true)
                                                          {:accessed true
                                                           :cause-card card}))}}})
  ...
  {:events [{:event :runner-turn-begins
   ...
   :abilities [{:action true
                ...
                :choices {:card #(and (installed? %)
                                      (has-subtype? % "Virus"))}
                :async true
                :effect (effect (add-counter state side eid target :virus (get-counters card :virus) nil))}]})
                                     ...
                                     {:prompt (str "Swap " (:title ice) " with another ice?")
                                      ...
                                      {:prompt "Choose another ice"
                                       ...
                                       :msg (msg "swap the positions of " (card-str state ice)
                                                 " and " (card-str state target))
                                       :effect (effect (swap-ice state side (get-card state ice) (get-card state target)))}}})
                                  card nil))}]}))
...
(defcard "K2CP Turbine"
  {:static-abilities [{:type :breaker-strength
                       :req (req (has-subtype? target "Icebreaker")
                                      (not (has-subtype? target "AI")))
                       :value 2}]
   :leave-play (effect (update-all-icebreakers state side))})
...
(defcard "Knight"
  (let [knight-req (effect (and (same-card? current-ice (get-nested-host card))
                             (<= (get-strength current-ice) (get-strength card))))]
    {:implementation "[Erratum] Program: Icebreaker - AI - Caïssa - Trojan"
     :abilities [{:action true
                  :label "Host on a piece of ice"
                  :async true
                  :effect (effect (let [k (get-card state card)
                                     hosted (ice? (:host k))
...
(defcard "Living Mural"
  ...
  {:on-install {:req (req (threat-level 4 state))
                ...
                :effect (effect (pump state side card 3 :end-of-turn))}
...
(defcard "Mammon"
  (auto-icebreaker {:flags {:runner-phase-12 (effect (pos? (:credit runner)))}
                    ...
                    :events [{:event :runner-turn-ends
                              :silent true
                              :effect (effect (update! state side (assoc-in card [:counter :power] 0)))}]}))
...
(defcard "Marjanah"
  (auto-icebreaker {:abilities [(break-sub 2 1 "Barrier"
                                           {:label "Break 1 Barrier subroutine"
                                            :break-cost-bonus (effect (when (:successful-run runner-reg) [(->c :credit -1)]))})
                                (strength-pump 1 1)]}))
                                ...
                                (register-events
                                  ...
                                  (let [broken-ice (:ice context)]
                                    [{:event :encounter-ice
                                      :duration :end-of-run
                                      :unregister-once-resolved true
                                      :effect (effect (doseq [sub (take 3 (:subroutines (:ice context)))]
  ...
  (let [break-abi {:label "Break X subroutines"
        ...
        host-abi {:action true
                  ...
                  :choices {:card #(and (in-hand? %)
                                        (= (:title %) "Matryoshka"))}
                  :msg (msg "host " (:title target) " on itself")
                  :effect (effect (host state :runner card target))}]
    (auto-icebreaker
      {:abilities [host-abi
                   break-abi
                   (strength-pump 1 1)]
       :events [{:event :runner-turn-begins
                 ...
                 :effect (effect (let [targets (filter #(facedown? %) (:hosted (get-card state card)))]
                                (doseq [oc targets]
...
(defcard "Maven"
  (auto-icebreaker {:abilities [(break-sub 2 1)]
                    :static-abilities [(breaker-strength-bonus (effect (count (filter program? (all-active-installed state :runner)))))]}))
...
(defcard "Medium"
  {:events [{:event :successful-run
            ...
            {:event :breach-server
             ...
             :effect (effect (continue-ability
                               state side {:req (req (< 1 (get-virus-counters state card)))
                                :prompt "How many additional cards from R&D do you want to access?"
                                :choices {:number (effect (dec (get-virus-counters state card)))
                                          :default (effect (dec (get-virus-counters state card)))}
                                :msg (msg "access " (quantify target "additional card") " from R&D")
                                :effect (effect (access-bonus state side :rd (max 0 target)))}
                               card nil))}]})
...
(defcard "Mongoose"
  (auto-icebreaker {:events [{:event :subroutines-broken
                              ...
                              :req (req (any-subs-broken-by-card? (:ice context) card)
                                             run)
                              :effect (effect (let [broken-ice (:ice context)]
                                             (register-lingering-effect
                                               state side
                                               card
...
(defcard "Monkeywrench"
  ...
  {:static-abilities [{:type :ice-strength
                       ...
                       :value (effect (if (same-card? target (:host card)) -2 -1))}]})
                                    ...
                                    (continue-ability
                                      ...
                                      {:prompt (msg "Choose a piece of ice to host " (:title target-card))
                                       :choices {:card #(and (installed? %)
                                                             (ice? %))}
                                       :async true
                                       :effect (effect (runner-install state side (assoc eid :source card :source-type :runner-install) target-card {:host-card (get-card state target) :msg-keys msg-keys}))}
                                      card nil))))
                              ;; declined to install
                              (effect-completed state side eid))))})]
    {:on-install {:async true
                  ...
                  :choices (effect ["Grip" "Stack"
                                 (when-not (zone-locked? state :runner :discard) "Heap")])
                  :msg (msg "search the " target " for a non-daemon program to install")
                  :effect (effect (continue-ability
                                    state side (muse-abi (if (= "Stack" target) :deck
                                                  (if (= "Grip" target) :hand :discard)))
                                    card nil))}}))

(defcard "Na'Not'K"
  (auto-icebreaker {:static-abilities [(breaker-strength-bonus (effect (count run-ices)))]
                    :abilities [(break-sub 1 1 "Sentry")
                                (strength-pump 3 2)]}))
  ...
  (let [self-rfg {:msg "remove itself from the game"
    ...
    (auto-icebreaker
      {:abilities [(break-sub 2 2 "All")
                   (strength-pump 1 1)]
       :move-zone-replacement (effect (let [old (:card context)
            ...
            {:event :breach-server
             ...
             :effect (effect (continue-ability
                               state side {:req (req (< 1 (get-virus-counters state card)))
                                ...
                                :effect (effect (access-bonus state side :hq (max 0 target)))}
                               card nil))}]})
...
(defcard "Origami"
  {:static-abilities [{:type :hand-size
                       :req (req (= :runner side))
                       :value (effect (count (filter #(= (:title %) "Origami")
                                                  (all-active-installed state :runner))))}]})

(defcard "Overmind"
  (auto-icebreaker
    {:on-install {:async true
                  :effect (effect (add-counter state side eid card :power (available-mu state) nil))}
...
(defcard "Parasite"
  ...
  {:implementation "[Erratum] Program: Virus - Trojan"
   :on-install
   {:effect (effect (when-let [h (:host card)]
  ...
  (letfn [(next-ice-inwards
    ...
    {:implementation "[Erratum] Program: Caïssa - Trojan"
     :events [{:event :successful-run
               ...
               :effect (effect (if (can-move-inwards? state card)
                              (continue-ability
                                state side
                                {:msg (msg "host itself on " (card-str state target))
                                 :effect (effect (host state side target card))}
                              ...
                              (continue-ability
                                ...
                                {:prompt "Choose another Caïssa to install"
                                 :show-discard true
                                 :choices {:req (req (has-subtype? target "Caïssa")
                                                          (or (in-hand? target)
...
(defcard "Persephone"
  (auto-icebreaker {:abilities [(break-sub 2 1 "Sentry")
                    ...
                    :events [{:event :pass-ice
                              ...
                              (effect
                                (continue-ability
                                  state side (let [fired-subs (count (filter :fired (:subroutines (:ice context))))]
                                    {:optional
...
(defcard "Pheromones"
  {:x-fn (effect (get-counters card :virus))
   :recurring (get-x-fn)
   :events [{:event :successful-run
             :silent true
             :req (req (= :hq (target-server context)))
...
(defcard "Plague"
  {:on-install {:prompt "Choose a server"
   ...
   :events [{:event :successful-run
             ...
             :async true
             :effect (effect (add-counter state :runner eid card :virus 2 nil))}]})
    ...
    (auto-icebreaker
      {:abilities [(break-sub 1 1 "Barrier")
                   ...
                   (let [base (strength-pump
                     ...
                     (assoc base :effect (effect (register-once state side (once card) card)
                                              ((:effect base) state side eid card targets))))]})))

(defcard "Principia"
  (auto-icebreaker
    {:install-cost-bonus (effect (- (count (filter #(has-subtype? % "Icebreaker") (all-installed state :runner)))))
     :abilities [(break-sub 1 1 "Barrier")
                 (strength-pump 2 2)]}))

(defcard "Progenitor"
  {:static-abilities [{:type :can-host
                       :req (req (program? target) (has-subtype? target "Virus"))
                       :no-mu true
                       :max-cards 1}
                      {:type :prevent-purge-virus-counters
                       :req (req (pos? (get-counters (first (:hosted card)) :virus)))
                       :value (effect {:card (first (:hosted card))
                                    :quantity 1})}]})
...
(defcard "Read-Write Share"
  (let [ab {:interactive (effect true)
            ...
            :async true
            :effect (effect (host state side (get-card state card) target {:facedown true})
    ...
    {:on-install ab
     ...
     :abilities [{:fake-cost [(->c :trash-can)]
                  ...
                  :interactive (effect true)
                  :async true
                  :effect (effect (if (seq (:hosted (get-card state card)))
                                 (let [set-aside-cards (set-aside-for-me state side eid (:hosted (get-card state card)))
                                   ...
                                   (continue-ability
                                     ...
                                     {:cost [(->c :trash-can)]
                                      ...
                                      :effect (effect (doseq [c set-aside-cards]
...
(defcard "Reaver"
  {:events [{:event :runner-trash
             ...
             :interactive (effect true)
             :once-per-instance true
             :req (req (some #(installed? (:card %)) targets)
...
(defcard "Rising Tide"
  (auto-icebreaker
    {:static-abilities [(breaker-strength-bonus (effect (count (filter #(has-subtype? % "Fracter") (:discard runner)))))]
     :abilities [(break-sub 1 1 "Barrier")
                 (strength-pump 1 1)]}))
                              ...
                              (let [guess (get-in card [:special :rng-guess])]
                                (when (#{(:cost (:accessed-card context)) (get-advancement-requirement (:accessed-card context))} guess)
                                  {:prompt "Choose one"
                                   ...
                                   :effect (effect (if (= target "Draw 2 cards")
                                                  (draw state :runner eid 2)
                                                  (gain-credits state :runner eid 3)))}))
                              card nil)))}
            {:event :post-access-card
             :effect (effect (update! state side (assoc-in card [:special :rng-guess] nil)))}
            (let [highest-cost
                  (fn [state card]
              ...
              {:event :successful-run
               ...
               {:req (req (#{:hq :rd} (target-server context))
                ...
                :yes-ability {:prompt "Guess a number"
                              :choices {:number (effect (highest-cost state card))}
                              :msg (msg "guess " target)
                              :effect (effect (update! state side (assoc-in card [:special :rng-guess] target)))}}})]
...
(defcard "Rook"
  {:implementation "[Erratum] Program: Caïssa - Trojan"
   :abilities [{:action true
                ...
                :async true
                :effect (effect (let [r (get-card state card)
                                   hosted? (ice? (:host r))
...
(defcard "Saci"
  ...
  {:events [{:event :rez
             ...
             :msg "gain 3 [Credits]"
             :async true
             :effect (effect (gain-credits state :runner eid 3))}
            {:event :derez
...
(defcard "Sadyojata"
  (swap-with-in-hand "Sadyojata"
                     {:req (req (<= 3 (count (:subtypes current-ice)))
                                     (<= (get-strength current-ice) (get-strength card)))}))
...
(defcard "Sang Kancil"
  (auto-icebreaker
    (let [discount-fn (effect (when (some #(and (event? %) (has-subtype? % "Run")) (:play-area runner))
                             [(->c :credit -2)]))]
      {:abilities [(break-sub 1 1 "Code Gate")
...
(defcard "Savoir-faire"
  {:abilities [{:cost [(->c :credit 2)]
                ...
                :once :per-turn
                :change-in-game-state {:req (req (not (install-locked? state side))
...
(defcard "Shibboleth"
  (auto-icebreaker {:x-fn (effect (if (threat-level 4 state) -2 0))
                    :abilities [(break-sub 1 1 "Code Gate")
                                (strength-pump 2 2)]
                    :static-abilities [(breaker-strength-bonus (get-x-fn))]}))
...
(defcard "Sipa"
  {:events [{:event :pass-ice
             ...
             :effect (effect
                       (continue-ability
                         state side (when-let [ice (get-card state (:ice context))]
                           {:prompt (str "Swap " (:title ice) " with another ice?")
                            :choices {:card #(and (installed? %)
                                                  (ice? %)
                                                  (not (same-card? % ice)))}
                            :msg (msg "swap the positions of " (card-str state ice)
                                      " and " (card-str state target))
                            :effect (effect (swap-ice state side ice (get-card state target)))})
                         card nil))}]})
                ...
                :effect (effect (let [initial-server target]
                                  (register-events state side card
                                    [{:event :pre-successful-run
                                      ...
                                      :prompt "Choose a server"
                                      :choices (effect ["Archives" "R&D" "HQ"])
                                      :msg (msg "change the attacked server to " target)
                                      :effect (effect (swap! state assoc-in [:run :server] [(unknown->kw target)]))}])
                ...
                :effect (effect (let [initial-server target]
                               (register-events state side card
                                 [{:event :pre-successful-run
                                   ...
                                   :req (req (= (unknown->kw initial-server) (-> run :server first)))
                                   :prompt "Choose a server"
                                   :choices (effect (cancellable remotes))
                                   :msg (msg "change the attacked server to " target)
                                   :effect (effect (swap! state assoc-in [:run :server] [(unknown->kw target)]))}])
                               (make-run state side eid initial-server card)))}]})
...
(defcard "Snowball"
  (auto-icebreaker {:abilities [(break-sub 1 1 "Barrier"
                                           {:additional-ability {:msg "gain +1 strength for the remainder of the run"
                                                                 :effect (effect (pump state side card 1 :end-of-run))}})
                                (strength-pump 1 1)]}))
  ...
  (letfn [(surf [state cice]
            {:prompt (msg "Choose a piece of ice before or after " (:title cice))
             ...
             :msg (msg "swap " (card-str state cice)
                       " and " (card-str state target))
             :effect (effect (let [tgtndx (card-index state target)]
    ...
    {:abilities [{:cost [(->c :credit 2)]
                  ...
                  :async true
                  :effect (effect (continue-ability state side (surf state current-ice) card nil))}]}))
...
(defcard "Takobi"
  {:special {:auto-place-counter :always}
   ...
   :abilities [{:req (req (get-current-encounter state))
                ...
                :effect (effect (pump state side target 3))}
...
(defcard "Tapwrm"
  (let [ability {:label "Gain [Credits] (start of turn)"
                 ...
                 :req (req (:runner-phase-12 @state))
                 :async true
                 :effect (effect (gain-credits state side eid (quot (:credit corp) 5)))}]
...
(defcard "Tracker"
  (let [ability {:prompt "Choose a server"
        ...
        prevent-sub {:event :pre-resolve-subroutine
                     ...
                     :unregister-once-resolved true
                     :req (req true)
                     :effect (effect (update-current-encounter state :prevent-subroutine true))
    ...
    {:abilities [{:action true
                  ...
                  :async true
                  :effect (effect (register-events state side card [prevent-sub])
                                  (make-run state side eid (:card-target card) card))}]
     :events [(assoc ability :event :runner-turn-begins)
              {:event :runner-turn-ends
               :silent true
...
(defcard "Tranquilizer"
  ...
  (let [action (effect (wait-for
                      ...
                      (if (and (rezzed? (get-card state (:host card)))
                        ...
                        (effect-completed state side eid))))]
    {:on-install {:interactive (effect true)
                  :async true
                  :effect action}
     :events [{:event :runner-turn-begins
               :async true
  ...
  (letfn [(credit-discount [s] (->> (all-installed-runner-type s :hardware)
    ...
    (auto-icebreaker {:abilities [(break-sub 3 2 "Barrier"
                                             {:label "Break up to 2 Barrier subroutine"
                                              :break-cost-bonus (effect [(->c :credit (max -3 (credit-discount state)))])})
                                  (strength-pump 2 2)]})))
...
(defcard "Trope"
  {:events [{:event :runner-turn-begins
   ...
   :abilities [{:action true
                ...
                (effect
                  (continue-ability
                    state side {:cost [(->c :click 1) (->c :remove-from-game)]
...
(defcard "Trypano"
  trojan
  (let [trash-if-5 (effect (let [h (get-card state (:host card))]
                          (if (and h
  ...
  (let [corp-draw {:optional {:prompt "Draw 1 card?"
    ...
    (auto-icebreaker {:implementation "Draw doesn't work when mixing breakers"
                      ...
                      :events [{:event :subroutines-broken
                                ...
                                :async true
                                :effect (effect (continue-ability state side runner-draw card nil))}]})))

(defcard "Unity"
  (auto-icebreaker {:abilities [(break-sub 1 1 "Code Gate")
                                (strength-pump 1 0 :end-of-encounter
                                               {:label "Add 1 strength for each installed icebreaker"
                                                :pump-bonus (effect (count (filter #(and (program? %)
                                                                                      (has-subtype? % "Icebreaker"))
                                                                                (all-active-installed state :runner))))})]}))
...
(defcard "Vamadeva"
  (swap-with-in-hand "Vamadeva"
                     {:req (req (= 1 (count (:subroutines current-ice)))
                                     (<= (get-strength current-ice) (get-strength card)))}))
...
(defcard "Wari"
  (letfn [(prompt-for-subtype []
            {:prompt "Choose one"
             :choices ["Barrier" "Code Gate" "Sentry"]
             :async true
             :effect (effect (wait-for (trash state side card {:unpreventable true
                                                            :cause-card card})
...
(defcard "World Tree"
  (let [search-and-install
    ...
    {:events [{:event :successful-run
               ...
               :msg (msg "trash " (:title target))
               :effect
               (effect (let [facedown-target (facedown? target)]
